...
/** Everything the FastAPI `CobraPoseAnalyzer` sends per frame. */
export interface CobraPoseData {
  ...
  smoothed_lift_ratio: number | null;
  back_arch_angle: number | null;
...
const EMPTY_RESULT: CobraPoseData = {
  pose_detected: false,
  view_mode: null,
  position_ok: false,
  ...
  ready: false,
  lift_ratio: null,
  smoothed_lift_ratio: null,
  back_arch_angle: null,
...
export default function useCobraPoseSocket() {
  const socketRef = useRef<WebSocket | null>(null);
  const [connected, setConnected] = useState(false);
  const [socketError, setSocketError] = useState<string | null>(null);
  const [result, setResult] = useState<CobraPoseData>(EMPTY_RESULT);
  ...
  const start = useCallback(
    (plan?: {
      ...
      try {
        ws = new WebSocket(
          `${WS_BASE}/ws/cobra_pose${query ? `?${query}` : ""}`,
        );
      } catch {
      ...
      ws.onmessage = (event) => {
        const data = JSON.parse(event.data) as CobraPoseData;
        setResult(data);
      };
