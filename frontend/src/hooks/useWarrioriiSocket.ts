...
/** Everything the FastAPI `WarriorIIAnalyzer` sends per frame. */
export interface WarriorIIData {
  pose_detected: boolean;
  front_leg: "left" | "right" | null;
  front_knee_angle: number | null;
  back_knee_angle: number | null;
  left_elbow_angle: number | null;
  right_elbow_angle: number | null;
  left_arm_tilt: number | null;
  right_arm_tilt: number | null;
  wingspan_ratio: number | null;
...
const EMPTY_RESULT: WarriorIIData = {
  pose_detected: false,
  front_leg: null,
...
export default function useWarriorIISocket() {
  const socketRef = useRef<WebSocket | null>(null);
  const [connected, setConnected] = useState(false);
  const [socketError, setSocketError] = useState<string | null>(null);
  const [result, setResult] = useState<WarriorIIData>(EMPTY_RESULT);
  ...
  const start = useCallback(
    (plan?: {
      ...
      try {
        ws = new WebSocket(
          `${WS_BASE}/ws/warrior_ii${query ? `?${query}` : ""}`,
        );
      } catch {
      ...
      ws.onmessage = (event) => {
        const data = JSON.parse(event.data) as WarriorIIData;
        setResult(data);
      };
