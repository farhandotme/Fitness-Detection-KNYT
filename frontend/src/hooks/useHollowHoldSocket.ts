...
/** Everything the FastAPI `HollowHoldAnalyzer` sends per frame. */
export interface HollowHoldData {
  pose_detected: boolean;
  active_side: "left" | "right" | null;
  shoulder_lift_deg: number | null;
  leg_lift_deg: number | null;
  knee_angle: number | null;
  elbow_angle: number | null;
  hip_rise_ratio: number | null;
...
const EMPTY_RESULT: HollowHoldData = {
  pose_detected: false,
  active_side: null,
  shoulder_lift_deg: null,
  leg_lift_deg: null,
  knee_angle: null,
  elbow_angle: null,
  hip_rise_ratio: null,
...
export default function useHollowHoldSocket() {
  const socketRef = useRef<WebSocket | null>(null);
  const [connected, setConnected] = useState(false);
  const [socketError, setSocketError] = useState<string | null>(null);
  const [result, setResult] = useState<HollowHoldData>(EMPTY_RESULT);
  ...
  const start = useCallback(
    (plan?: {
      ...
      try {
        ws = new WebSocket(
          `${WS_BASE}/ws/hollow_hold${query ? `?${query}` : ""}`,
        );
      } catch {
      ...
      ws.onmessage = (event) => {
        const data = JSON.parse(event.data) as HollowHoldData;
        setResult(data);
      };
