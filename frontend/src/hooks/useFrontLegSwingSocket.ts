...
/** Everything the FastAPI `FrontLegSwingAnalyzer` sends per frame. */
export interface FrontLegSwingData {
...
const EMPTY_RESULT: FrontLegSwingData = {
  pose_detected: false,
  view_mode: null,
  position_ok: false,
...
export default function useFrontLegSwingSocket() {
  const socketRef = useRef<WebSocket | null>(null);
  const [connected, setConnected] = useState(false);
  const [socketError, setSocketError] = useState<string | null>(null);
  const [result, setResult] = useState<FrontLegSwingData>(EMPTY_RESULT);
  ...
  const start = useCallback(
    (plan?: {
      ...
      try {
        ws = new WebSocket(
          `${WS_BASE}/ws/front_leg_swing${query ? `?${query}` : ""}`,
        );
      } catch {
      ...
      ws.onmessage = (event) => {
        const data = JSON.parse(event.data) as FrontLegSwingData;
        setResult(data);
