...
export type SupportMode = "free" | "wall" | "strap";

/** Everything the FastAPI `DancerPoseAnalyzer` sends per frame. */
export interface DancerPoseData {
  ...
  front_arm_reach: number | null;
  hand_foot_gap: number | null;
  front_arm_reach_ok: boolean;
  hand_foot_connected: boolean;
  balance_confidence: number | null;
  support_mode: SupportMode;
  wall_supported: boolean;
  strap_supported: boolean;
...
const EMPTY_RESULT: DancerPoseData = {
  pose_detected: false,
  standing_side: null,
...
export default function useDancerPoseSocket() {
  const socketRef = useRef<WebSocket | null>(null);
  const [connected, setConnected] = useState(false);
  const [socketError, setSocketError] = useState<string | null>(null);
  const [result, setResult] = useState<DancerPoseData>(EMPTY_RESULT);
  ...
  const start = useCallback(
    (plan?: {
      ...
      if (plan?.supportMode != null)
        params.set("support_mode", plan.supportMode);
      const query = params.toString();

      let ws: WebSocket;
      try {
        ws = new WebSocket(
          `${WS_BASE}/ws/dancer_pose${query ? `?${query}` : ""}`,
        );
      } catch {
      ...
      ws.onmessage = (event) => {
        const data = JSON.parse(event.data) as DancerPoseData;
        setResult(data);
      };
