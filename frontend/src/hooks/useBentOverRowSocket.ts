...
/** Everything the FastAPI `BentOverRowAnalyzer` sends per frame. */
export interface BentOverRowData {
  ...
  back_flat: boolean;
  knees_soft: boolean;
  elbows_tracking: boolean;
  stayed_hinged: boolean;
...
const EMPTY_RESULT: BentOverRowData = {
  pose_detected: false,
  in_position: false,
...
export default function useBentOverRowSocket() {
  const socketRef = useRef<WebSocket | null>(null);
  const [connected, setConnected] = useState(false);
  const [socketError, setSocketError] = useState<string | null>(null);
  const [result, setResult] = useState<BentOverRowData>(EMPTY_RESULT);
  ...
  const start = useCallback(
    (plan?: {
      ...
      try {
        ws = new WebSocket(
          `${WS_BASE}/ws/bent_over_row${query ? `?${query}` : ""}`,
        );
      } catch {
      ...
      ws.onmessage = (event) => {
        const data = JSON.parse(event.data) as BentOverRowData;
        setResult(data);
