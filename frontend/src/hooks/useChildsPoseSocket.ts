...
/** Everything the FastAPI `ChildsPoseAnalyzer` sends per frame. */
export interface ChildsPoseData {
  pose_detected: boolean;
  view_mode: "side" | "front" | "angled" | null;
  ready: boolean;
  is_calibrated: boolean;
  calibration_progress: number;
  tabletop_baseline: number | null;
  fold_ratio: number | null;
  smoothed_fold_ratio: number | null;
...
const EMPTY_RESULT: ChildsPoseData = {
  pose_detected: false,
  view_mode: null,
  ready: false,
  is_calibrated: false,
  calibration_progress: 0,
  tabletop_baseline: null,
  fold_ratio: null,
  smoothed_fold_ratio: null,
  chest_fold: null,
...
export default function useChildsPoseSocket() {
  const socketRef = useRef<WebSocket | null>(null);
  const [connected, setConnected] = useState(false);
  const [socketError, setSocketError] = useState<string | null>(null);
  const [result, setResult] = useState<ChildsPoseData>(EMPTY_RESULT);
  ...
  const start = useCallback(
    (plan?: {
      ...
      try {
        ws = new WebSocket(
          `${WS_BASE}/ws/childs_pose${query ? `?${query}` : ""}`,
        );
      } catch {
      ...
      ws.onmessage = (event) => {
        const data = JSON.parse(event.data) as ChildsPoseData;
        setResult(data);
      };
