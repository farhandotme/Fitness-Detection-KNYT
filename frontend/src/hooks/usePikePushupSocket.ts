...
/** Everything the FastAPI `PikePushupAnalyzer` sends per frame. */
export interface PikePushupData {
...
const EMPTY_RESULT: PikePushupData = {
  pose_detected: false,
  view_mode: null,
  position_ok: false,
...
export default function usePikePushupSocket() {
  const socketRef = useRef<WebSocket | null>(null);
  const [connected, setConnected] = useState(false);
  const [socketError, setSocketError] = useState<string | null>(null);
  const [result, setResult] = useState<PikePushupData>(EMPTY_RESULT);
  ...
  const start = useCallback(
    (plan?: {
      ...
      try {
        ws = new WebSocket(
          `${WS_BASE}/ws/pike_pushup${query ? `?${query}` : ""}`,
        );
      } catch {
      ...
      ws.onmessage = (event) => {
        const data = JSON.parse(event.data) as PikePushupData;
        setResult(data);
