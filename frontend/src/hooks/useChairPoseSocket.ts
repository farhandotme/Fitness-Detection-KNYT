...
/** Everything the FastAPI `ChairPoseAnalyzer` sends per frame. */
export interface ChairPoseData {
...
const EMPTY_RESULT: ChairPoseData = {
  pose_detected: false,
  knee_angle: null,
  hip_angle: null,
  torso_angle: null,
...
export default function useChairPoseSocket() {
  const socketRef = useRef<WebSocket | null>(null);
  const [connected, setConnected] = useState(false);
  const [socketError, setSocketError] = useState<string | null>(null);
  const [result, setResult] = useState<ChairPoseData>(EMPTY_RESULT);
  ...
  const start = useCallback(
    (plan?: {
      ...
      try {
        ws = new WebSocket(
          `${WS_BASE}/ws/chair_pose${query ? `?${query}` : ""}`,
        );
      } catch {
      ...
      ws.onmessage = (event) => {
        const data = JSON.parse(event.data) as ChairPoseData;
        setResult(data);
      };
