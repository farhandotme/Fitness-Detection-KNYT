...
/** Everything the FastAPI `SupermanAnalyzer` sends per frame. */
export interface SupermanData {
  pose_detected: boolean;
  view_mode: "side" | "front" | "angled" | null;
  lift: number | null;
  smoothed_lift: number | null;
  chest_rise: number | null;
  leg_rise: number | null;
...
const EMPTY_RESULT: SupermanData = {
  pose_detected: false,
  view_mode: null,
  lift: null,
  smoothed_lift: null,
...
export default function useSupermanSocket() {
  const socketRef = useRef<WebSocket | null>(null);
  const [connected, setConnected] = useState(false);
  const [socketError, setSocketError] = useState<string | null>(null);
  const [result, setResult] = useState<SupermanData>(EMPTY_RESULT);
  ...
  const start = useCallback(
    (plan?: { targetSeconds?: number; targetSets?: number; setNumber?: number }) => {
      ...
      let ws: WebSocket;
      try {
        ws = new WebSocket(`${WS_BASE}/ws/superman${query ? `?${query}` : ""}`);
      } catch {
      ...
      ws.onmessage = (event) => {
        const data = JSON.parse(event.data) as SupermanData;
        setResult(data);
      };
