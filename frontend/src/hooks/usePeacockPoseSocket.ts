...
/** Everything the FastAPI `PeacockPoseAnalyzer` sends per frame. */
export interface PeacockPoseData {
  pose_detected: boolean;
  active_side: "left" | "right" | null;
  elbow_angle: number | null;
  elbow_tuck_ratio: number | null;
  body_line_angle: number | null;
...
const EMPTY_RESULT: PeacockPoseData = {
  pose_detected: false,
  active_side: null,
  elbow_angle: null,
  elbow_tuck_ratio: null,
  body_line_angle: null,
...
export default function usePeacockPoseSocket() {
  const socketRef = useRef<WebSocket | null>(null);
  const [connected, setConnected] = useState(false);
  const [socketError, setSocketError] = useState<string | null>(null);
  const [result, setResult] = useState<PeacockPoseData>(EMPTY_RESULT);
  ...
  const start = useCallback(
    (plan?: {
      ...
      try {
        ws = new WebSocket(
          `${WS_BASE}/ws/peacock_pose${query ? `?${query}` : ""}`,
        );
      } catch {
      ...
      ws.onmessage = (event) => {
        const data = JSON.parse(event.data) as PeacockPoseData;
        setResult(data);
      };
