...
/** Everything the FastAPI `StandingForwardFoldAnalyzer` sends per frame. */
export interface StandingForwardFoldData {
  pose_detected: boolean;
  active_side: "left" | "right" | null;
  fold_angle: number | null;
...
const EMPTY_RESULT: StandingForwardFoldData = {
  pose_detected: false,
  active_side: null,
  fold_angle: null,
  knee_angle: null,
...
export default function useStandingForwardFoldSocket() {
  const socketRef = useRef<WebSocket | null>(null);
  const [connected, setConnected] = useState(false);
  const [socketError, setSocketError] = useState<string | null>(null);
  const [result, setResult] = useState<StandingForwardFoldData>(EMPTY_RESULT);
  ...
  const start = useCallback(
    (plan?: {
      ...
      try {
        ws = new WebSocket(
          `${WS_BASE}/ws/standing_forward_fold${query ? `?${query}` : ""}`,
        );
      } catch {
      ...
      ws.onmessage = (event) => {
        const data = JSON.parse(event.data) as StandingForwardFoldData;
        setResult(data);
      };
