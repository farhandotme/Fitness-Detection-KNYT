...
export function useExerciseSocket(exercise: ExerciseConfig) {
  ...
  const [lastRep, setLastRep] = useState<{ rep_form_quality: string | null, rep_classification: string | null, rep_duration: number | null, feedback: string | null } | null>(null);

  const socketRef = useRef<WebSocket | null>(null);

  const start = useCallback((params: { targetReps?: number; targetSeconds?: number; targetSets?: number; setNumber?: number }) => {
    ...
    if (params.targetSets) searchParams.set("target_sets", params.targetSets.toString());
    if (params.setNumber) searchParams.set("set_number", params.setNumber.toString());
