...
/** Everything the FastAPI `PlankHoldAnalyzer` sends per frame. */
...
const EMPTY_RESULT: PlankHoldData = {
  pose_detected: false,
  active_side: null,
...
export default function usePlankHoldSocket() {
  const socketRef = useRef<WebSocket | null>(null);
  const [connected, setConnected] = useState(false);
  const [socketError, setSocketError] = useState<string | null>(null);
  const [result, setResult] = useState<PlankHoldData>(EMPTY_RESULT);
  ...
  const start = useCallback(
    (plan?: {
      ...
      try {
        ws = new WebSocket(
          `${WS_BASE}/ws/plank_hold${query ? `?${query}` : ""}`,
        );
      } catch {
      ...
      ws.onmessage = (event) => {
        const data = JSON.parse(event.data) as PlankHoldData;
        setResult(data);
      };
