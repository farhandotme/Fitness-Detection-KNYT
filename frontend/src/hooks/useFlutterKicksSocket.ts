...
/** Everything the FastAPI `FlutterKicksAnalyzer` sends per frame. */
export interface FlutterKicksData {
  ...
  /** Backend-computed "is this leg currently the raised one" — always use
   * these instead of re-deriving from the raw angles on the frontend,
   * since the backend's relative-comparison logic (not a fixed absolute
   * threshold) is what actually decides this. */
  left_leg_up: boolean;
  right_leg_up: boolean;
  /** Which leg is currently confirmed elevated — the source of truth for
   * which leg the user should swap to next. */
  elevated_leg: "left" | "right" | null;
  stage: string;
  /** Total counted alternation swaps — this is the backend's rep count. */
  rep_count: number;
  /** rep_count // 2 — full left+right pairs, handy for a "cycles" display. */
  cycle_count: number;
  good_reps: number;
  flawed_reps: number;
  left_reps: number;
...
const EMPTY_RESULT: FlutterKicksData = {
  pose_detected: false,
  view_mode: null,
  position_ok: false,
...
export default function useFlutterKicksSocket() {
  const socketRef = useRef<WebSocket | null>(null);
  const [connected, setConnected] = useState(false);
  const [socketError, setSocketError] = useState<string | null>(null);
  const [result, setResult] = useState<FlutterKicksData>(EMPTY_RESULT);
  ...
  const [lastCompletedRep, setLastCompletedRep] = useState({
    ...
    elevated_leg: null as "left" | "right" | null,
  ...
  const start = useCallback(
    (plan?: {
      ...
      try {
        ws = new WebSocket(
          `${WS_BASE}/ws/flutter_kicks${query ? `?${query}` : ""}`,
        );
      } catch {
      ...
      ws.onmessage = (event) => {
        const data = JSON.parse(event.data) as FlutterKicksData;
        setResult(data);
        ...
        if (data.rep_completed) {
          setLastCompletedRep({
            ...
            elevated_leg: data.elevated_leg,
