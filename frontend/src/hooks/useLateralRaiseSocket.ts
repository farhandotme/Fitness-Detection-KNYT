...
/** Everything the FastAPI `LateralRaiseAnalyzer` sends per frame. */
export interface LateralRaiseData {
  pose_detected: boolean;
  left_abduction_angle: number | null;
  right_abduction_angle: number | null;
  angle: number | null;
  lift: number | null;
...
const EMPTY_RESULT: LateralRaiseData = {
  pose_detected: false,
  left_abduction_angle: null,
  right_abduction_angle: null,
  angle: null,
  lift: null,
  smoothed_lift: null,
  stage: "down",
...
export default function useLateralRaiseSocket() {
  const socketRef = useRef<WebSocket | null>(null);
  const [connected, setConnected] = useState(false);
  const [socketError, setSocketError] = useState<string | null>(null);
  const [result, setResult] = useState<LateralRaiseData>(EMPTY_RESULT);
  ...
  const start = useCallback(
    (plan?: {
      ...
      try {
        ws = new WebSocket(
          `${WS_BASE}/ws/lateral_raise${query ? `?${query}` : ""}`,
        );
      } catch {
      ...
      ws.onmessage = (event) => {
        const data = JSON.parse(event.data) as LateralRaiseData;
        setResult(data);
