...
export default function useRepWebSocket(mode: string, count: number = 10) {
  ...
  useEffect(() => {
    ...
    socket.current = new WebSocket(
      `${import.meta.env.VITE_WEBSOCKET_FASTAPI_URL}/ws/bicep_curl_${mode}_arm`,
    );
  ...
  return { connected, result, lastCompletedRep, sendFrame };
}
