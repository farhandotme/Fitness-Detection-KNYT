...
/** Everything the FastAPI `CalfRaiseAnalyzer` sends per frame. */
export interface CalfRaiseData {
  ...
  left_lift: number | null;
  right_lift: number | null;
...
const EMPTY_RESULT: CalfRaiseData = {
  pose_detected: false,
  position_ok: false,
...
export default function useCalfRaiseSocket() {
  const socketRef = useRef<WebSocket | null>(null);
  const [connected, setConnected] = useState(false);
  const [socketError, setSocketError] = useState<string | null>(null);
  const [result, setResult] = useState<CalfRaiseData>(EMPTY_RESULT);
  ...
  const start = useCallback(
    (plan?: {
      ...
      try {
        ws = new WebSocket(
          `${WS_BASE}/ws/calf_raise${query ? `?${query}` : ""}`,
        );
      } catch {
      ...
      ws.onmessage = (event) => {
        const data = JSON.parse(event.data) as CalfRaiseData;
        setResult(data);
