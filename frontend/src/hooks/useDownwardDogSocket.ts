...
/** Everything the FastAPI `DownwardDogAnalyzer` sends per frame. */
export interface DownwardDogData {
  pose_detected: boolean;
  active_side: "left" | "right" | null;
  hip_fold_angle: number | null;
  elevation_ratio: number | null;
  arm_line_angle: number | null;
...
const EMPTY_RESULT: DownwardDogData = {
  pose_detected: false,
  active_side: null,
  hip_fold_angle: null,
  elevation_ratio: null,
  arm_line_angle: null,
  elbow_angle: null,
  knee_angle: null,
  head_angle: null,
...
export default function useDownwardDogSocket() {
  const socketRef = useRef<WebSocket | null>(null);
  const [connected, setConnected] = useState(false);
  const [socketError, setSocketError] = useState<string | null>(null);
  const [result, setResult] = useState<DownwardDogData>(EMPTY_RESULT);
  ...
  const start = useCallback(
    (plan?: {
      ...
      try {
        ws = new WebSocket(
          `${WS_BASE}/ws/downward_dog${query ? `?${query}` : ""}`,
        );
      } catch {
      ...
      ws.onmessage = (event) => {
        const data = JSON.parse(event.data) as DownwardDogData;
        setResult(data);
      };
