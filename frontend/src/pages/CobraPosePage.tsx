...
import useCobraPoseSocket from "../hooks/useCobraPoseSocket";
import CobraPoseCamera from "../conponents/CobraPoseCamera";
import CobraPoseStatsPanel from "../conponents/CobraPoseStatsPanel";
import "./BicepPage.css";
import "./CobraPosePage.css";
...
function CobraPosePage() {
  ...
  // ---- advance a set once the BACKEND confirms this set's hold time is
  // met ---- `session_complete` is computed server-side
  // (CobraPoseAnalyzer._is_complete), from the target_seconds the backend
  // itself was given when the socket opened. We never derive this from
  // currentHold/holdTarget on the client.
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    //   POST /api/workouts/complete { exercise: "cobra_pose", holdTarget, totalSets }
  ...
  return (
    <div className="bicep-page cobra-page">
      <div className="bicep-header">
        <div className="bicep-header-left">
          <h1 className="bicep-title">Cobra Pose Trainer</h1>
        </div>
        ...
        <div className="bicep-stats-col">
          {phase === "setup" && (
            ...
            <div className="session-builder">
              ...
              <div className="cobra-setup-tip">
                Lie face down on the floor: legs extended behind you, hands
                under your shoulders. Press up to lift your chest and arch
                your back, keeping your hips and thighs pressed to the
                floor, and hold. The timer only runs while your form checks
                out, and it never loses progress you've already earned — if
                you drop out of the pose it just pauses until you're back in
                position.
              </div>
          ...
          {phase === "active" && (
            <div className="single-arm-wrap">
              <CobraPoseStatsPanel data={result} />
