...
import useLungeSocket from "../hooks/useLungeSocket";
import LungeCamera from "../conponents/LungeCamera";
import LungeStatsPanel from "../conponents/LungeStatsPanel";
import "./BicepPage.css";
import "./LungePage.css";
...
function LungePage() {
  ...
  // `session_complete` is computed server-side (LungeAnalyzer._is_complete),
  ...
  useEffect(() => {
    ...
    //   POST /api/workouts/complete { exercise: "lunge", repsPerSet, totalSets }
  ...
  const totals = useMemo(() => {
    return setSummaries.reduce(
      ...
      { reps: 0, good: 0, flawed: 0, time: 0, left: 0, right: 0 },
    );
  }, [setSummaries]);
  ...
  return (
    <div className="bicep-page lunge-page">
      <div className="bicep-header">
        <div className="bicep-header-left">
          <button className="lunge-back-btn" onClick={() => navigate("/exercises")}>
            ← Library
          </button>
          <h1 className="bicep-title">Lunge Trainer</h1>
        </div>
        ...
        <div className="bicep-stats-col">
          {phase === "setup" && (
            ...
            <div className="session-builder">
              ...
              <div className="lunge-setup-tip">
                Stand tall, facing the camera, with your whole body — head to
                feet — in frame. Reps count from either leg, so alternate
                freely; the coach will nudge you if one side falls behind.
              </div>
