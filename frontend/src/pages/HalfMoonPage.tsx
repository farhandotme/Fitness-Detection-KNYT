...
import useHalfMoonSocket from "../hooks/useHalfMoonSocket";
import type { SupportMode } from "../hooks/useHalfMoonSocket";
import HalfMoonCamera from "../conponents/HalfMoonCamera";
import HalfMoonStatsPanel from "../conponents/HalfMoonStatsPanel";
import "./HalfMoonPage.css";
...
const SUPPORT_OPTIONS: { mode: SupportMode; label: string; hint: string }[] = [
  { mode: "free", label: "Free-standing", hint: "No support — classic version" },
  { mode: "wall", label: "Wall-assisted", hint: "Back or hand near a wall" },
  { mode: "block", label: "Block-assisted", hint: "Bottom hand on a yoga block" },
];

const INSTRUCTION_STEPS = [
  { icon: "🦶", label: "Stand on one leg." },
  { icon: "🦵", label: "Lift the other leg out." },
  { icon: "🫁", label: "Open your chest." },
  { icon: "🙌", label: "Reach the top arm up." },
];
...
function HalfMoonPage() {
  ...
  // `session_complete` is computed server-side (HalfMoonAnalyzer._is_complete),
  ...
  useEffect(() => {
    ...
    //   POST /api/workouts/complete { exercise: "half-moon-pose", holdTarget, totalSets }
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [phase, restRemaining, start, currentSet, holdTarget, totalSets, supportMode]);
  ...
  return (
    <div className="bicep-page halfmoon-page">
      <div className="bicep-header">
        <div className="bicep-header-left">
          <button
            className="halfmoon-back-btn"
            onClick={() => navigate("/exercises")}
          >
            ← Library
          </button>
          <h1 className="bicep-title">Half Moon Pose Trainer</h1>
        </div>
      ...
      <div className="halfmoon-instructions">
        {INSTRUCTION_STEPS.map((step) => (
          <div key={step.label} className="halfmoon-instruction-item">
            <span className="halfmoon-instruction-icon">{step.icon}</span>
            <span className="halfmoon-instruction-label">{step.label}</span>
          </div>
        ))}
      </div>
        ...
        <div className="bicep-stats-col">
          {phase === "setup" && (
            ...
            <div className="session-builder">
              <div className="builder-row">
                ...
                <div className="halfmoon-support-options">
                  {SUPPORT_OPTIONS.map((opt) => (
                    <button
                      key={opt.mode}
                      className={`halfmoon-support-btn ${supportMode === opt.mode ? "active" : ""}`}
                      onClick={() => setSupportMode(opt.mode)}
                    >
                      <span className="halfmoon-support-btn-label">
                        {opt.label}
                      </span>
                      <span className="halfmoon-support-btn-hint">
                        {opt.hint}
                      </span>
                    </button>
                  ))}
              ...
              <div className="halfmoon-setup-tip">
                Stand on one leg, lift the other leg out to the side, and
                open your chest as you reach your top arm up — your bottom
                hand can float, rest on the floor, or rest on a block. A
                little wobble is completely normal and won't stop the
                timer — only losing the shape (foot down, leg dropped, or
                folding all the way over) pauses it.
              </div>
          ...
          {phase === "resting" && (
            <div className="rest-panel">
              ...
              <p className="halfmoon-rest-tip">
                Switch to your other side for the next set so both legs get
                a turn balancing.
              </p>
          ...
          {phase === "active" && (
            <div className="single-arm-wrap">
              <HalfMoonStatsPanel data={result} />
