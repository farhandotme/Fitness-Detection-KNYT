...
import usePikePushupSocket from "../hooks/usePikePushupSocket";
import PikePushupCamera from "../conponents/PikePushupCamera";
import PikePushupStatsPanel from "../conponents/PikePushupStatsPanel";
import "./PikePushupPage.css";
...
function PikePushupPage() {
  ...
  // `session_complete` is computed server-side (PikePushupAnalyzer._is_complete),
  ...
  useEffect(() => {
    ...
    //   POST /api/workouts/complete { exercise: "pike_pushup", repsPerSet, totalSets }
    // That endpoint is what should write the "user completed this exercise"
    // record to the database. The frontend must only ever send what the
    // backend already validated here (exercise_complete === true) — it
    // must not decide completion on its own.
    console.log(
  ...
  return (
    <div className="pike-page">
      <div className="pike-header">
        <div className="pike-header-left">
          <h1 className="pike-title">Pike Push-up Trainer</h1>
        </div>

        <div className="pike-header-right">
          {phase === "active" && (
      ...
      {socketError && <div className="pike-error">{socketError}</div>}
      {cameraError && <div className="pike-error">{cameraError}</div>}

      <div className="pike-body">
        <div className="pike-camera-col">
          <PikePushupCamera
            active={phase === "active"}
            sendFrame={sendFrame}
            skeleton={skeleton}
        ...
        <div className="pike-stats-col">
          {phase === "setup" && (
            ...
            <div className="session-builder">
              ...
              <div className="pike-setup-tip">
                Get into the pike position: hands and feet on the floor, hips
                lifted high into an inverted V (like the top of a downward dog),
                arms straight. Bend your elbows to lower your head toward the
                floor, then press back up. Camera to your side works best — reps
                only count once a real pike position is confirmed and stay
                uncounted if you drop into a flat plank or stand up mid-rep.
              </div>
          ...
          {phase === "active" && (
            <div className="single-arm-wrap">
              <PikePushupStatsPanel data={result} />
