...
import useBirdDogSocket from "../hooks/useBirdDogSocket";
import BirdDogCamera from "../conponents/BirdDogCamera";
import BirdDogStatsPanel from "../conponents/BirdDogStatsPanel";
import "./BirdDogPage.css";
...
function BirdDogPage() {
  ...
  // `session_complete` is computed server-side (BirdDogAnalyzer._is_complete),
  // from the target_reps the backend itself was given when the socket opened.
  // We never derive this from currentReps/repsPerSet on the client — and we
  // never count a same-side or partial reach as a rep on the client either;
  // rep_count only ever moves when the backend's anti-cheat gate passed it.
  useEffect(() => {
    ...
    const summary: SetSummary = {
      ...
      rejectedReps: result.rejected_reps ?? 0,
  ...
  useEffect(() => {
    ...
    //   POST /api/workouts/complete { exercise: "bird-dog", repsPerSet, totalSets }
  ...
  const sessionRejected = result.rejected_reps ?? 0;
  ...
  const totals = useMemo(() => {
    return setSummaries.reduce(
      (acc, s) => ({
        ...
        rejected: acc.rejected + s.rejectedReps,
        time: acc.time + s.elapsedTime,
      }),
      { reps: 0, good: 0, flawed: 0, rejected: 0, time: 0 },
    );
  }, [setSummaries]);
  ...
  return (
    <div className="bicep-page bird-dog-page">
      <div className="bicep-header">
        <div className="bicep-header-left">
          ...
          <h1 className="bicep-title">Bird Dog Trainer</h1>
        </div>
            ...
            <>
              ...
              <div className="session-summary">
                ...
                <div className="session-summary-item">
                  ...
                  <span className="v">{sessionRejected}</span>
                </div>
                <div className="session-summary-item">
        ...
        <div className="bicep-stats-col">
          {phase === "setup" && (
            ...
            <div className="session-builder">
              ...
              <div className="bird-dog-setup-tip">
                Get on all fours, side-on to the camera — hands under
                shoulders, knees under hips, back flat. Reach one arm and
                the OPPOSITE leg straight out together, hold for a beat,
                then return to tabletop and switch sides. Same-side raises,
                partial reaches and rushed flicks won't be counted — the
                camera checks all of it automatically.
              </div>
              ...
              {setSummaries[setSummaries.length - 1] && (
                <div className="arm-grid rest-panel-grid">
                  ...
                  <div className="arm-grid-item">
                    <span className="k">Not counted</span>
                    <span className="v">
                      {setSummaries[setSummaries.length - 1].rejectedReps}
                    </span>
                  </div>
                </div>
              )}
          ...
          {phase === "active" && (
            <div className="single-arm-wrap">
              <BirdDogStatsPanel data={result} />

              {(lastRepEvent.feedback || result.feedback) && (
                <div
                  className={`feedback-box ${lastRepEvent.rep_form_quality ?? ""} ${
                    lastRepEvent.was_rejected ? "rejected" : ""
                  }`}
                >
                  <strong>
                    {lastRepEvent.was_rejected ? "Not Counted" : "Coach Feedback"}
                  </strong>
                  <p>{lastRepEvent.feedback ?? result.feedback}</p>
                </div>
              )}
            </div>
          )}
