...
import useDancerPoseSocket from "../hooks/useDancerPoseSocket";
import type { SupportMode } from "../hooks/useDancerPoseSocket";
import DancerPoseCamera from "../conponents/DancerPoseCamera";
import DancerPoseStatsPanel from "../conponents/DancerPoseStatsPanel";
import "./DancerPosePage.css";
...
const HOLD_PRESETS = [10, 15, 20, 30];
const SET_PRESETS = [1, 2, 3, 4];
const REST_PRESETS = [20, 30, 45, 60];

const SUPPORT_OPTIONS: { mode: SupportMode; label: string; hint: string }[] = [
  {
    mode: "free",
    label: "Free-standing",
    hint: "No support — classic version",
  },
  { mode: "wall", label: "Wall-assisted", hint: "Near a wall for balance" },
  {
    mode: "strap",
    label: "Strap-assisted",
    hint: "Strap bridging hand to foot",
  },
];

const INSTRUCTION_STEPS = [
  { icon: "🦶", label: "Stand on one leg." },
  { icon: "🦵", label: "Bend the other knee back." },
  { icon: "🤲", label: "Reach back and catch your foot." },
  { icon: "🙆", label: "Kick into your hand, lift your chest." },
];
...
function DancerPosePage() {
  const navigate = useNavigate();

  const [holdTarget, setHoldTarget] = useState(15);
  const [totalSets, setTotalSets] = useState(2);
  const [restSeconds, setRestSeconds] = useState(30);
  const [supportMode, setSupportMode] = useState<SupportMode>("free");
  ...
  // `session_complete` is computed server-side (DancerPoseAnalyzer._is_complete),
  ...
  useEffect(() => {
    ...
    //   POST /api/workouts/complete { exercise: "dancer-pose", holdTarget, totalSets }
  ...
  return (
    <div className="bicep-page dancer-page">
      <div className="bicep-header">
        <div className="bicep-header-left">
          <button
            className="dancer-back-btn"
            onClick={() => navigate("/exercises")}
          >
            ← Library
          </button>
          <h1 className="bicep-title">Dancer Pose Trainer</h1>
        </div>
      ...
      <div className="dancer-instructions">
        {INSTRUCTION_STEPS.map((step) => (
          <div key={step.label} className="dancer-instruction-item">
            <span className="dancer-instruction-icon">{step.icon}</span>
            <span className="dancer-instruction-label">{step.label}</span>
          </div>
        ))}
      </div>
      ...
      <div className="bicep-body">
        ...
        <div className="bicep-stats-col">
          {phase === "setup" && (
            ...
            // here) rather than being user-editable. Whatever's picked
            // here is what actually gets sent to the backend as
            // target_seconds/target_sets/support_mode — and the backend
            // (not this component) decides when it's been met.
            <div className="session-builder">
              <div className="builder-row">
                <span className="builder-label">Support</span>
                <div className="dancer-support-options">
                  {SUPPORT_OPTIONS.map((opt) => (
                    <button
                      key={opt.mode}
                      className={`dancer-support-btn ${supportMode === opt.mode ? "active" : ""}`}
                      onClick={() => setSupportMode(opt.mode)}
                    >
                      <span className="dancer-support-btn-label">
                        {opt.label}
                      </span>
                      <span className="dancer-support-btn-hint">
                        {opt.hint}
                      </span>
                    </button>
                  ))}
              ...
              <div className="dancer-setup-tip">
                Stand on one leg, bend the other knee back, and reach the
                same-side hand back to catch your foot or ankle — kick into your
                hand as you lift your chest and reach the front arm forward. A
                little wobble is completely normal and won't stop the timer —
                only losing the shape (leg straightens or drops, or folding all
                the way forward) pauses it.
              </div>
          ...
          {phase === "resting" && (
            <div className="rest-panel">
              ...
              <p className="dancer-rest-tip">
                Switch to your other side for the next set so both legs get a
                turn balancing.
              </p>
          ...
          {phase === "active" && (
            <div className="single-arm-wrap">
              <DancerPoseStatsPanel data={result} />
