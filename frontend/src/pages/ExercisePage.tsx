...
import { ArrowLeft, Target, RefreshCw, Clock, Play, ShieldCheck, Sparkles, Minus, Plus } from 'lucide-react';
...
export function ExercisePage() {
  ...
  if (!match || !exercise) {
    return <div className="p-8 text-center text-destructive">Exercise not found.</div>;
  }

  const handleStart = () => {
    // Navigate to session passing params in URL state or query
    // Simple way with Wouter: use URL search params
    setLocation(`/exercise/${exercise.id}/session?target=${target}&sets=${sets}&rest=${rest}`);
  };
  ...
  const NumberField = ({
    ...
    <div className="rounded-2xl border border-border bg-background/60 p-4 transition-colors focus-within:border-primary/55">
      <div className="mb-3 flex items-center justify-between">
        ...
        <span className="font-mono text-[10px] uppercase tracking-widest text-primary/70">{min}–{max}</span>
      </div>
      <div className="flex items-center gap-2">
        ...
        <div className="relative flex-1">
          <input
            ...
            aria-label={label}
          />
          <span className="pointer-events-none absolute right-4 top-1/2 -translate-y-1/2 font-mono text-xs uppercase tracking-widest text-muted-foreground">{suffix}</span>
        </div>
        <button
          type="button"
  ...
  return (
      ...
      <header className="sticky top-0 z-50 bg-background/80 backdrop-blur-xl border-b border-border p-4">
        <div className="max-w-3xl mx-auto flex items-center gap-4">
          <Link href="/" data-testid="link-back-home" className="p-2.5 bg-secondary/70 hover:bg-secondary rounded-full transition-colors">
            <ArrowLeft className="w-5 h-5 text-foreground" />
          </Link>
          <h1 className="text-sm font-bold uppercase tracking-[.2em] text-muted-foreground">Session setup</h1>
        </div>
      </header>
        ...
        {/* Header Hero */}
         <div className="bg-card border border-card-border rounded-[2rem] p-6 md:p-10 mb-6 flex flex-col md:flex-row gap-8 items-center md:items-stretch overflow-hidden relative">
           <div className="w-full md:w-1/2 min-h-[210px] rounded-2xl overflow-hidden relative bg-[#173b42]">
              {exercise.imageUrl ? <img src={exercise.imageUrl} alt="" className="absolute inset-0 w-full h-full object-cover opacity-85" /> : <div className="absolute inset-0 bg-[radial-gradient(circle_at_70%_20%,hsl(var(--accent)/.7),transparent_30%),linear-gradient(135deg,#173b42,#2d6e6b)]" />}
             <div className="absolute inset-0 bg-gradient-to-t from-[#173b42]/80 to-transparent" />
             <div className="absolute left-5 bottom-5 text-[#f2f5ed]">
               <div className="text-[10px] uppercase tracking-[.24em] text-[#f2b35b] font-bold">Today's focus</div>
               <div className="text-2xl font-display font-extrabold mt-1">{exercise.mode === 'reps' ? 'Controlled strength' : 'Steady endurance'}</div>
             </div>
           </div>
           <div className="flex-1 self-center">
             <div className="flex items-center gap-2 text-primary text-xs uppercase tracking-[.2em] font-bold mb-4"><Sparkles className="w-4 h-4" /> Guided session</div>
             <h2 className="text-4xl md:text-5xl font-black tracking-[-.05em] leading-none mb-3">{exercise.name}</h2>
             <p className="text-lg text-muted-foreground mb-6 max-w-md">{exercise.tagline}</p>
        ...
        {/* Setup Tip */}
         <div className="bg-accent/10 border border-accent/25 rounded-2xl p-5 mb-6 flex gap-4 items-start">
           ...
           <div>
           <h3 className="text-accent font-bold uppercase tracking-widest text-xs mb-2">Coach's cue</h3>
          <p className="text-foreground/80">{exercise.setupTip}</p>
           </div>
        </div>

        {/* Configuration Form */}
         <div className="space-y-6 bg-card border border-card-border rounded-[1.5rem] p-6 md:p-8 shadow-sm">
          <h3 className="text-xl font-bold tracking-tight border-b border-border pb-4">Session Parameters</h3>
