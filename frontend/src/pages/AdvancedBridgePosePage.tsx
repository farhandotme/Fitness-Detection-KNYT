...
import AdvancedBridgePoseCamera from "../conponents/AdvancedBridgePoseCamera";
import AdvancedBridgePoseStatsPanel from "../conponents/AdvancedBridgePoseStatsPanel";
import "../exercise-shared.css";
import "./AdvancedBridgePosePage.css";
import useAdvancedBridgePoseSocket from "../conponents/useAdvancedBridgePoseSocket";
...
function AdvancedBridgePosePage() {
  ...
  // (AdvancedBridgePoseAnalyzer._is_complete), from the target_seconds the
  // backend itself was given when the socket opened. We never derive this
  // from currentHold/holdTarget on the client.
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    //   POST /api/workouts/complete { exercise: "advanced_bridge_pose", holdTarget, totalSets }
  ...
  const sessionGood = result.good_seconds ?? 0;
  const sessionFlawed = result.flawed_seconds ?? 0;
  ...
  const totals = useMemo(() => {
    return setSummaries.reduce(
      ...
      { hold: 0, good: 0, flawed: 0, breaks: 0 },
    );
  }, [setSummaries]);
    ...
    <div className="bridge-page">
      <div className="bridge-header">
        <div className="bridge-header-left">
          <h1 className="bridge-title">Advanced Bridge Pose Trainer</h1>
        </div>
      ...
      {socketError && <div className="bridge-error">{socketError}</div>}
      {cameraError && <div className="bridge-error">{cameraError}</div>}

      <div className="bridge-body">
        <div className="bridge-camera-col">
          <AdvancedBridgePoseCamera
            active={phase === "active"}
            sendFrame={sendFrame}
            skeleton={skeleton}
            ...
            <>
              ...
              <div className="session-summary">
                <div className="session-summary-item good">
                  ...
                  <span className="v">{formatSeconds(sessionGood)}</span>
                </div>
                <div className="session-summary-item flawed">
                  <span className="k">Flawed</span>
                  <span className="v">{formatSeconds(sessionFlawed)}</span>
                </div>
        ...
        <div className="bridge-stats-col">
          {phase === "setup" && (
            // TODO(coach-assignment): once a coach/plan API exists,
            // holdTarget and totalSets should come from the user's
            ...
            <div className="session-builder">
              ...
              <div className="builder-total">
                ...
                <strong>{totalSets * holdTarget}s total hold</strong>
              </div>

              <div className="bridge-setup-tip">
                Set up side-on to the camera, whole body visible. Press up from
                the floor onto both hands and both feet, arms and legs
                extending, pushing your hips up high — they need to be the
                highest point of your body, arched well above your shoulders and
                knees, like a dome. The timer only runs while that arch is
                confirmed — it pauses the instant your hips drop or a hand or
                foot lifts.
              </div>
          ...
          {phase === "active" && (
            <div className="single-arm-wrap">
              <AdvancedBridgePoseStatsPanel data={result} />

              {result.feedback && (
                <div className={`feedback-box ${result.hold_quality ?? ""}`}>
          ...
          {phase === "complete" && (
            <div className="results-panel">
              ...
              <div className="results-table">
                <div className="results-row results-head bridge-results-row">
                  <span>Set</span>
                  <span>Held</span>
                  <span>Best</span>
                  <span>Breaks</span>
                ...
                {setSummaries.map((s) => (
                  <div
                    key={s.setNumber}
                    className="results-row bridge-results-row"
                  >
                    <span>{s.setNumber}</span>
                    <span>{formatSeconds(s.holdSeconds)}</span>
                    <span className="good-text">
                      {formatSeconds(s.bestStreak)}
                    </span>
...
export default AdvancedBridgePosePage;
