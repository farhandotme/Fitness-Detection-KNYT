...
import useLegRaiseSocket, {
  type LegRaiseVariation,
} from "../hooks/useLegRaiseSocket";
import LegRaiseCamera from "../conponents/LegRaiseCamera";
import LegRaiseStatsPanel from "../conponents/LegRaiseStatsPanel";
import "../pages/BicepPage.css";
import "./LegRaisePage.css";

// Shoulders, hips, knees, ankles — both sides, so the overlay draws the
// full leg (pushup/plank only need the upper-body + a single leg line).
...
function LegRaisePage() {
  ...
  // `session_complete` is computed server-side (LegRaiseAnalyzer._is_complete),
  ...
  useEffect(() => {
    ...
    //   POST /api/workouts/complete { exercise: "leg-raise", repsPerSet, totalSets }
  ...
  return (
    <div className="bicep-page legraise-page">
      <div className="bicep-header">
        <div className="bicep-header-left">
          <h1 className="bicep-title">Double Leg Raise Trainer</h1>
        </div>
        ...
        <div className="bicep-stats-col">
          {phase === "setup" && (
            ...
            <div className="session-builder">
              ...
              <div className="builder-row">
                <span className="builder-label">Leg variation</span>
                <div className="builder-controls">
                  <div className="reps-presets">
                    <button
                      className={`reps-preset ${variation === "straight" ? "active" : ""}`}
                      onClick={() => setVariation("straight")}
                    >
                      Straight legs
                    </button>
                    <button
                      className={`reps-preset ${variation === "bent_knees" ? "active" : ""}`}
                      onClick={() => setVariation("bent_knees")}
                    >
                      Bent knees (beginner)
                    </button>
                  </div>
              ...
              <div className="legraise-setup-tip">
                Lie on your back, legs extended and together, arms by your sides
                or tucked under your glutes. Lift both legs together, keep them
                straight (or use the bent-knees variation below), and lower
                slowly under control — reps only count once a confirmed
                lying-flat position is detected. Camera positioned to the side
                or front/top-oblique so both legs stay visible.
              </div>
          ...
          {phase === "active" && (
            <div className="single-arm-wrap">
              <LegRaiseStatsPanel data={result} />
