...
import useWallSitSocket from "../hooks/useWallSitSocket";
import WallSitCamera from "../conponents/WallSitCamera";
import WallSitStatsPanel from "../conponents/WallSitStatsPanel";
import "./WallSitPage.css";
...
const INSTRUCTION_STEPS = [
  { icon: "🧱", label: "Back flat to the wall." },
  { icon: "⬇️", label: "Slide down." },
  { icon: "📐", label: "Thighs near parallel." },
  { icon: "🦵", label: "Knees over ankles." },
  { icon: "⏱️", label: "Hold." },
];
...
function WallSitPage() {
  ...
  // `session_complete` is computed server-side (WallSitAnalyzer._is_complete),
  ...
  useEffect(() => {
    ...
    //   POST /api/workouts/complete { exercise: "wall-sit", holdTarget, totalSets }
  ...
  return (
    <div className="bicep-page wallsit-page">
      <div className="bicep-header">
        <div className="bicep-header-left">
          <button
            className="wallsit-back-btn"
            onClick={() => navigate("/exercises")}
          >
            ← Library
          </button>
          <h1 className="bicep-title">Wall Sit Trainer</h1>
        </div>
      ...
      <div className="wallsit-instructions">
        {INSTRUCTION_STEPS.map((step) => (
          <div key={step.label} className="wallsit-instruction-item">
            <span className="wallsit-instruction-icon">{step.icon}</span>
            <span className="wallsit-instruction-label">{step.label}</span>
          </div>
        ))}
      </div>
        ...
        <div className="bicep-stats-col">
          {phase === "setup" && (
            ...
            <div className="session-builder">
              ...
              <div className="wallsit-setup-tip">
                Stand facing the camera and lean your back flat against a wall
                behind you, then slide down until your thighs are near parallel
                to the floor — knees stacked over your ankles. The timer only
                runs while your form checks out, and it never loses progress
                you've already earned — if you stand up or slide too low it just
                pauses until you're back in the hold.
              </div>
