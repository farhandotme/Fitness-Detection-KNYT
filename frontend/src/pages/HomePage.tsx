...
export function HomePage() {
  ...
  const filteredExercises = exercises.filter((e) => {
    ...
    const matchesSearch =
      !normalizedSearch ||
      [e.name, e.tagline, e.category, e.difficulty].some((value) =>
      ...
      <main className="max-w-6xl mx-auto p-4 mt-6 flex flex-col gap-7">
        <section className="relative overflow-hidden rounded-4xl bg-[#0d2028] p-7 md:p-10 text-[#f4f7f2] shadow-2xl shadow-black/25 border border-primary/15">
          ...
          <div className="relative max-w-xl">
            <div className="flex items-center gap-2 text-accent text-xs uppercase tracking-[.22em] font-bold">
              <Sparkles className="w-4 h-4" /> Focused movement
            </div>
            ...
            <p className="mt-5 text-sm md:text-base text-slate-300 max-w-md leading-relaxed">
              A precise coach for the work in front of you. Choose an exercise,
              set your pace, and let your form do the talking.
            </p>
          </div>
        </section>
        ...
        <div className="flex items-center justify-between text-xs text-muted-foreground">
          <span>
            ...
            {filteredExercises.length === 1 ? "exercise" : "exercises"} shown
          </span>
          {searchTerm && (
            <button
          ...
          {filteredExercises.map((ex) => (
            ...
            >
              <div
                ...
                className="bg-card border border-card-border rounded-3xl p-5 h-full min-h-61.25 transition-all duration-300 hover:border-primary/50 hover:-translate-y-1 hover:shadow-xl hover:shadow-primary/10 relative overflow-hidden flex flex-col justify-between"
              >
                <div className="absolute right-0 top-0 z-0 h-36 w-44 overflow-hidden rounded-bl-[4rem] bg-linear-to-br from-primary/15 via-accent/10 to-transparent">
                  {ex.imageUrl ? (
                    <img
                      src={ex.imageUrl}
                      alt=""
                      className="h-full w-full object-cover opacity-45 mix-blend-multiply transition-opacity duration-300 group-hover:opacity-65"
                    />
                  ) : (
                ...
                <div className="mb-8">
                  ...
                  <h2 className="text-2xl font-bold tracking-tight mb-1 group-hover:text-primary transition-colors">
                    {ex.name}
                  </h2>
