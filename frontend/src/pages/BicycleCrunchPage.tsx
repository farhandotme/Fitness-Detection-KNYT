...
const POSE_CONNECTIONS: [number, number][] = [
  ...
  [24, 26],
  [26, 28],
  [14, 25], // right elbow -> left knee (the crossover being measured)
  [13, 26], // left elbow -> right knee
];

const REP_PRESETS = [10, 16, 20, 30];
...
function BicycleCrunchPage() {
  ...
  useEffect(() => {
    ...
    //   POST /api/workouts/complete { exercise: "bicycle_crunch", repsPerSet, totalSets }
    console.log(
  ...
  return (
    <div className="bcrunch-page">
      <div className="bcrunch-header">
        <div className="bcrunch-header-left">
          <h1 className="bcrunch-title">Bicycle Crunch Trainer</h1>
        </div>

        <div className="bcrunch-header-right">
          {phase === "active" && (
      ...
      <div className="bcrunch-body">
        <div className="bcrunch-camera-col">
          ...
          {phase === "active" && (
            <>
              <div className="bcrunch-progress-track">
                <div
                  className="bcrunch-progress-fill"
                  style={{ width: `${progressPct}%` }}
                />
              </div>
              <div className="bcrunch-summary-row">
        ...
        <div className="bcrunch-stats-col">
          {phase === "setup" && (
            // TODO(coach-assignment): once a coach/plan API exists, repsPerSet
            // and totalSets should come from the user's assigned plan, and
            // this picker should become read-only. The backend (not this
            // component) is what decides when the plan has actually been met.
            <div className="session-builder">
              <div className="builder-row">
                <span className="builder-label">Reps per set (left+right)</span>
              ...
              <div className="bcrunch-setup-tip">
                Lie on your back, hands lightly behind your head, knees pulled
                toward your chest. Bring one elbow toward the opposite knee
                while the other leg extends out, then switch — each rep only
                counts once you cross over clearly to one side, then switch to
                the other side while your legs keep pedaling.
              </div>
          ...
          {phase === "active" && (
            <div className="single-arm-wrap">
              <BicycleCrunchStatsPanel data={result} />

              {(lastEvent.feedback || result.feedback) && (
                <div
                  className={`feedback-box ${lastEvent.wasFullRep ? "good" : ""}`}
                >
              ...
              <div className="results-table">
                ...
                {setSummaries.map((s) => (
                  <div key={s.setNumber} className="results-row">
                    ...
                    <span>{s.reps}</span>
                    <span>{s.leftCount}</span>
                    <span>{s.rightCount}</span>
...
export default BicycleCrunchPage;
