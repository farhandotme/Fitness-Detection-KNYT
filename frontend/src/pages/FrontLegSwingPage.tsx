...
import useFrontLegSwingSocket from "../hooks/useFrontLegSwingSocket";
import FrontLegSwingCamera from "../conponents/FrontLegSwingCamera";
import FrontLegSwingStatsPanel from "../conponents/FrontLegSwingStatsPanel";
import "../exercise-shared.css";
import "./FrontLegSwingPage.css";
...
function FrontLegSwingPage() {
  const [repsPerSet, setRepsPerSet] = useState(12);
  ...
  // `session_complete` is computed server-side (FrontLegSwingAnalyzer._is_complete),
  ...
  useEffect(() => {
    ...
    //   POST /api/workouts/complete { exercise: "front_leg_swing", repsPerSet, totalSets }
  ...
  return (
    <div className="frontswing-page">
      <div className="frontswing-header">
        <div className="frontswing-header-left">
          <h1 className="frontswing-title">Front Leg Swing Trainer</h1>
        </div>

        <div className="frontswing-header-right">
          {phase === "active" && (
      ...
      {socketError && <div className="frontswing-error">{socketError}</div>}
      {cameraError && <div className="frontswing-error">{cameraError}</div>}

      <div className="frontswing-body">
        <div className="frontswing-camera-col">
          <FrontLegSwingCamera
            active={phase === "active"}
            sendFrame={sendFrame}
            skeleton={skeleton}
        ...
        <div className="frontswing-stats-col">
          {phase === "setup" && (
            ...
            <div className="session-builder">
              <div className="builder-row">
                <span className="builder-label">Reps per set (per leg)</span>
              ...
              <div className="frontswing-setup-tip">
                Stand side-on to the camera, whole body visible — put your
                nearest leg to the camera as the one you're swinging. Plant the
                far leg, swing the near leg forward and back through the air,
                staying upright. Reps only count once the working leg swings out
                far enough, actually leaves the ground, and returns to rest — a
                confirmed side-on position is required to start counting.
              </div>
          ...
          {phase === "resting" && (
            <div className="rest-panel">
              ...
              <div className="frontswing-rest-tip">
                Switching legs? Turn around so the other leg is nearest the
                camera before the next set starts — the working leg is detected
                automatically from whichever is more visible.
              </div>
          ...
          {phase === "active" && (
            <div className="single-arm-wrap">
              <FrontLegSwingStatsPanel data={result} />
...
export default FrontLegSwingPage;
