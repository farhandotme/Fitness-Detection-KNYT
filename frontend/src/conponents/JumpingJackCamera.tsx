...
/**
 * Same capture pipeline as SquatCamera, scoped to its own class names
 * (`jj-viewfinder*`) so it can be styled independently — jumping jacks
 * need a wider frame margin since the arms swing out past the shoulders.
 */
      ...
      {active ? (
        <>
          <video
            ...
            muted
            playsInline
            className="jj-viewfinder-video"
          />
          <canvas ref={overlayRef} className="jj-viewfinder-overlay" />
          <canvas ref={canvasRef} style={{ display: "none" }} />
        </>
      ) : (
        <div className="jj-viewfinder-placeholder">
          <span>📷</span>
          <p>
            Camera turns on when you hit Start — step back so your arms and legs
            both stay in frame
          </p>
        </div>
      )}
