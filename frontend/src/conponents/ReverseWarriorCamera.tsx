...
import type { Landmark } from "../hooks/useReverseWarriorSocket";
 ...
 * taller, front-on resolution — Reverse Warrior needs the WHOLE body
 * visible at once (both arms overhead/down, both legs in a wide stance,
 * head to feet), unlike a plank's side-on profile view.
 */
export default function ReverseWarriorCamera({
  active,
  sendFrame,
  skeleton,
  onError,
      ...
      try {
        stream = await navigator.mediaDevices.getUserMedia({
          video: {
            facingMode: "user",
            width: { ideal: 480 },
            height: { ideal: 854 }, // portrait-leaning — need full body, head to feet
      ...
      {active ? (
        <>
          <video
            ...
            muted
            playsInline
            className="rw-viewfinder-video"
          />
          <canvas ref={overlayRef} className="rw-viewfinder-overlay" />
          <canvas ref={canvasRef} style={{ display: "none" }} />
        </>
      ) : (
        <div className="rw-viewfinder-placeholder">
          <span>📷</span>
          <p>
            Camera turns on when you hit Start — stand facing the camera with
            room for a wide lunge stance, head to feet in frame
          </p>
        </div>
      )}
    </div>
