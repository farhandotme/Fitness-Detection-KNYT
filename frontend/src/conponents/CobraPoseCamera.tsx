...
import type { Landmark } from "../hooks/useCobraPoseSocket";
...
/**
 * Cobra Pose is filmed lying on the floor, so a side-on placement (phone
 * propped on the floor, body horizontal across the frame) shows the back
 * arch best — same landscape-leaning 4:3 feed as the push-up camera, which
 * works for either a side or front placement.
 */
export default function CobraPoseCamera({
  active,
  sendFrame,
  skeleton,
  onError,
      ...
      try {
        stream = await navigator.mediaDevices.getUserMedia({
          video: {
            facingMode: "user",
            width: { ideal: 854 },
            height: { ideal: 640 }, // landscape-leaning — works for side or front cobra pose framing
  ...
  return (
    <div className="cobra-viewfinder">
      {active ? (
        <>
          <video
            ref={videoRef}
            ...
            muted
            playsInline
            className="cobra-viewfinder-video"
          />
          <canvas ref={overlayRef} className="cobra-viewfinder-overlay" />
          <canvas ref={canvasRef} style={{ display: "none" }} />
        </>
      ) : (
        <div className="cobra-viewfinder-placeholder">
          <span>🐍</span>
          <p>
            Camera turns on when you hit Start — lie face down, filmed from
