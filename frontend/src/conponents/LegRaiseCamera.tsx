...
import type { Landmark } from "../hooks/useLegRaiseSocket";
...
/**
 * Same capture/overlay plumbing as `PushupCamera` — landscape-leaning
 * feed so it works whether the phone is propped to film from the side
 * (profile, recommended — the hip angle reads most cleanly) or set up
 * front/top-oblique so both legs are clearly separated in frame.
 */
export default function LegRaiseCamera({
  active,
  sendFrame,
  skeleton,
  onError,
      ...
      try {
        stream = await navigator.mediaDevices.getUserMedia({
          video: {
            facingMode: "user",
            width: { ideal: 854 },
            height: { ideal: 640 }, // landscape-leaning — works for side or front leg-raise framing
  ...
  return (
    <div className="legraise-viewfinder">
      {active ? (
        <>
          <video
            ref={videoRef}
            ...
            muted
            playsInline
            className="legraise-viewfinder-video"
          />
          <canvas ref={overlayRef} className="legraise-viewfinder-overlay" />
          <canvas ref={canvasRef} style={{ display: "none" }} />
        </>
      ) : (
        <div className="legraise-viewfinder-placeholder">
          <span>📷</span>
          <p>
            Camera turns on when you hit Start — lie on your back, filmed from
            the side or front so both legs are visible
          </p>
        </div>
      )}
    </div>
