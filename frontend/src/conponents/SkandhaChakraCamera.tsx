...
import type { Landmark } from "../hooks/useSkandhaChakraSocket";
...
/**
 * Same capture/overlay plumbing as `ArnoldPressCamera`. Portrait
 * framing, front-on, with extra headroom and side-room called out
 * specifically — the arms sweep a full circle out to the sides and
 * overhead here, so both matter more than for a press.
 */
export default function SkandhaChakraCamera({
  active,
  sendFrame,
  skeleton,
  onError,
      ...
      try {
        stream = await navigator.mediaDevices.getUserMedia({
          video: {
            facingMode: "user",
            width: { ideal: 640 },
            height: { ideal: 854 }, // portrait-leaning — full body, room for arms to circle
  ...
  return (
    <div className="chakra-viewfinder">
      {active ? (
        <>
          <video
            ref={videoRef}
            ...
            muted
            playsInline
            className="chakra-viewfinder-video"
          />
          <canvas ref={overlayRef} className="chakra-viewfinder-overlay" />
          <canvas ref={canvasRef} style={{ display: "none" }} />
        </>
      ) : (
        <div className="chakra-viewfinder-placeholder">
          <span>📷</span>
          <p>
            Camera turns on when you hit Start — stand front-on with space
            on both sides and above your head for the full circle
          </p>
        </div>
      )}
    </div>
