...
import type { Landmark } from "../hooks/useDancerPoseSocket";
...
/**
 * Same capture pipeline as the other exercise cameras. Dancer Pose reaches
 * both forward (front arm) and up-and-back (kicked leg + head), so a
 * slightly tall-of-square frame works better than the very wide plank
 * cameras or the strict portrait wall-sit camera — somewhere in between.
 */
export default function DancerPoseCamera({
  active,
  sendFrame,
  skeleton,
  onError,
  ...
  return (
    <div className="dancer-viewfinder">
      {active ? (
        <>
          <video
            ref={videoRef}
            ...
            muted
            playsInline
            className="dancer-viewfinder-video"
          />
          <canvas ref={overlayRef} className="dancer-viewfinder-overlay" />
          <canvas ref={canvasRef} style={{ display: "none" }} />
        </>
      ) : (
        <div className="dancer-viewfinder-placeholder">
          <span>💃</span>
          <p>
            Camera turns on when you hit Start — stand facing the camera
            with room to kick a leg back and reach an arm forward, full
            body in frame
          </p>
        </div>
      )}
