...
import type { Landmark } from "../hooks/useJabSocket";
...
/**
 * Requests the camera's natural resolution instead of forcing a portrait
 * crop — most webcams are landscape (16:9 or 4:3) sensors, so demanding a
 * 3:4 portrait stream forced the browser to crop into the sensor, which
 * looked like an aggressive, unwanted zoom. The viewfinder box below
 * adapts to whatever aspect ratio the camera actually reports, and the
 * video is displayed with `object-fit: contain` so the *entire* camera
 * frame is always visible — nothing cropped, nothing zoomed in.
 */
export default function JabCamera({
  active,
  sendFrame,
  skeleton,
  onError,
  ...
  return (
    <div className="jab-viewfinder" ref={viewfinderRef}>
      {active ? (
        <>
          <video
            ref={videoRef}
            ...
            muted
            playsInline
            className="jab-viewfinder-video"
          />
          <canvas ref={overlayRef} className="jab-viewfinder-overlay" />
          <canvas ref={canvasRef} style={{ display: "none" }} />
        </>
      ) : (
        <div className="jab-viewfinder-placeholder">
          <span>🥊</span>
          <p>
            Camera turns on when you hit Start — stand facing the camera in a
            boxing stance, hands up near your chin
          </p>
        </div>
      )}
    </div>
