...
import type { Landmark } from "../hooks/useLateralRaiseSocket";
 ...
 * `lateralraise-viewfinder` class names so it can live alongside the other
 * exercise pages without style collisions.
 */
export default function LateralRaiseCamera({
  active,
  sendFrame,
  skeleton,
  onError,
  ...
  return (
    <div className="lateralraise-viewfinder">
      {active ? (
        <>
          <video
            ref={videoRef}
            ...
            muted
            playsInline
            className="lateralraise-viewfinder-video"
          />
          <canvas ref={overlayRef} className="lateralraise-viewfinder-overlay" />
          <canvas ref={canvasRef} style={{ display: "none" }} />
        </>
      ) : (
        <div className="lateralraise-viewfinder-placeholder">
          <span>📷</span>
          <p>
            Camera turns on when you hit Start — step back so your whole
            torso and both arms fit out to the sides in frame
          </p>
        </div>
      )}
    </div>
