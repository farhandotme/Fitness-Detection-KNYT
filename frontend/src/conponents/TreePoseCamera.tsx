...
import type { Landmark } from "../hooks/useTreePoseSocket";
 ...
 * portrait-biased resolution — tree pose is judged from a front-on,
 * full-body standing view, so the useful frame is tall (head-to-feet
 * across the height), not wide like the plank/side-plank cameras.
 */
export default function TreePoseCamera({
  active,
  sendFrame,
  skeleton,
  onError,
      ...
      try {
        stream = await navigator.mediaDevices.getUserMedia({
          video: {
            facingMode: "user",
            width: { ideal: 480 },
            height: { ideal: 854 }, // portrait — tree pose is judged front-on, standing
      ...
      {active ? (
        <>
          <video
            ...
            muted
            playsInline
            className="tree-viewfinder-video"
          />
          <canvas ref={overlayRef} className="tree-viewfinder-overlay" />
          <canvas ref={canvasRef} style={{ display: "none" }} />
        </>
      ) : (
        <div className="tree-viewfinder-placeholder">
          <span>🌳</span>
          <p>
            Camera turns on when you hit Start — stand facing the camera,
            full body in frame, ready to balance on one leg
          </p>
        </div>
      )}
    </div>
