...
import type { Landmark } from "../hooks/usePushupSocket";
...
/**
 * Always opens whatever camera the browser considers the default. Push-ups
 * are commonly filmed either from the side (phone propped on the floor,
 * body horizontal across the frame) or from the front (camera at floor
 * level facing the user), so — unlike the portrait-biased squat camera —
 * this requests a roughly 4:3 landscape-leaning feed that works for either
 * placement, and lets the CSS box flip to portrait on narrow screens.
 */
export default function PushupCamera({
      ...
      try {
        stream = await navigator.mediaDevices.getUserMedia({
          video: {
            facingMode: "user",
            width: { ideal: 854 },
            height: { ideal: 640 }, // landscape-leaning — works for side or front push-up framing
  ...
  return (
    <div className="pushup-viewfinder">
      {active ? (
        <>
          <video
            ref={videoRef}
        ...
        <div className="pushup-viewfinder-placeholder">
          <span>📷</span>
          <p>
            Camera turns on when you hit Start — get down into plank
            position, filmed from the side or the front
          </p>
        </div>
      )}
    </div>
