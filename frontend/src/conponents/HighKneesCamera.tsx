...
import type { Landmark } from "../hooks/useHighKneesSocket";
 ...
 * `highknees-viewfinder` class names so it can live alongside the other
 * exercise pages without style collisions.
 */
export default function HighKneesCamera({
  active,
  sendFrame,
  skeleton,
  onError,
  ...
  return (
    <div className="highknees-viewfinder">
      {active ? (
        <>
          <video
            ref={videoRef}
            ...
            muted
            playsInline
            className="highknees-viewfinder-video"
          />
          <canvas ref={overlayRef} className="highknees-viewfinder-overlay" />
          <canvas ref={canvasRef} style={{ display: "none" }} />
        </>
      ) : (
        <div className="highknees-viewfinder-placeholder">
          <span>📷</span>
          <p>
            Camera turns on when you hit Start — step back so your whole
            body, head to feet, fits in frame
          </p>
        </div>
      )}
    </div>
