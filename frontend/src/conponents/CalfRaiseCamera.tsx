...
import type { Landmark } from "../hooks/useCalfRaiseSocket";
...
/**
 * Calf raises need the whole body — head to feet — in frame, filmed
 * head-on and full length, unlike the push-up's landscape-leaning shot.
 * This requests a portrait-leaning feed so a phone propped up vertically
 * a few feet away captures feet and head at once.
 */
export default function CalfRaiseCamera({
  active,
  sendFrame,
  skeleton,
  onError,
      ...
      try {
        stream = await navigator.mediaDevices.getUserMedia({
          video: {
            facingMode: "user",
            width: { ideal: 640 },
            height: { ideal: 854 }, // portrait-leaning — full body, head to feet
  ...
  return (
    <div className="calf-raise-viewfinder">
      {active ? (
        <>
          <video
            ref={videoRef}
            ...
            muted
            playsInline
            className="calf-raise-viewfinder-video"
          />
          <canvas ref={overlayRef} className="calf-raise-viewfinder-overlay" />
          <canvas ref={canvasRef} style={{ display: "none" }} />
        </>
      ) : (
        <div className="calf-raise-viewfinder-placeholder">
          <span>📷</span>
          <p>
            Camera turns on when you hit Start — stand facing the camera,
            far enough back that your head and feet are both in frame
          </p>
        </div>
      )}
    </div>
