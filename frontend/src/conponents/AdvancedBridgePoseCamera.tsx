...
import type { Landmark } from "./useAdvancedBridgePoseSocket";
...
/**
 * Advanced bridge pose (wheel pose) is judged side-on, with the whole
 * arched body — hands to feet — stretched across the frame, the same
 * wide, landscape-leaning framing as hollow hold and front leg swing.
 */
export default function AdvancedBridgePoseCamera({
  active,
  sendFrame,
  skeleton,
  onError,
      ...
      try {
        stream = await navigator.mediaDevices.getUserMedia({
          video: {
            facingMode: "user",
            width: { ideal: 960 },
            height: { ideal: 540 }, // wide landscape — full arched body, side-on
  ...
  return (
    <div className="bridge-viewfinder">
      {active ? (
        <>
          <video
            ref={videoRef}
            ...
            muted
            playsInline
            className="bridge-viewfinder-video"
          />
          <canvas ref={overlayRef} className="bridge-viewfinder-overlay" />
          <canvas ref={canvasRef} style={{ display: "none" }} />
        </>
      ) : (
        <div className="bridge-viewfinder-placeholder">
          <span>📷</span>
          <p>
            Camera turns on when you hit Start — set up side-on, whole
            body visible, ready to press up into the bridge
          </p>
        </div>
      )}
    </div>
