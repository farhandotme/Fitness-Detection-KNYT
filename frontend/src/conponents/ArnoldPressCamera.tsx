...
import type { Landmark } from "../hooks/useArnoldPressSocket";
...
/**
 * Same capture/overlay plumbing as `SingleLegSquatCamera`. Portrait
 * framing, front-on — same as the squat camera — but with extra vertical
 * room built into the placeholder copy specifically: arms go straight up
 * overhead here, so headroom above the head matters more than it does
 * for a squat.
 */
export default function ArnoldPressCamera({
  active,
  sendFrame,
  skeleton,
  onError,
      ...
      try {
        stream = await navigator.mediaDevices.getUserMedia({
          video: {
            facingMode: "user",
            width: { ideal: 640 },
            height: { ideal: 854 }, // portrait-leaning — full body plus headroom for pressing overhead
  ...
  return (
    <div className="arnold-viewfinder">
      {active ? (
        <>
          <video
            ref={videoRef}
            ...
            muted
            playsInline
            className="arnold-viewfinder-video"
          />
          <canvas ref={overlayRef} className="arnold-viewfinder-overlay" />
          <canvas ref={canvasRef} style={{ display: "none" }} />
        </>
      ) : (
        <div className="arnold-viewfinder-placeholder">
          <span>📷</span>
          <p>
            Camera turns on when you hit Start — stand or sit front-on, with
            room above your head for the press
          </p>
        </div>
      )}
    </div>
