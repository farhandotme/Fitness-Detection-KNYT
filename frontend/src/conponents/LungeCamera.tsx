...
import type { Landmark } from "../hooks/useLungeSocket";
...
/**
 * Same capture pipeline as SquatCamera/JumpingJackCamera — always opens the
 * browser's default camera, no device picker — scoped to its own
 * `lunge-viewfinder` class names so it can live alongside the other
 * exercise pages without style collisions.
 */
export default function LungeCamera({
  active,
  sendFrame,
  skeleton,
  onError,
  ...
  return (
    <div className="lunge-viewfinder">
      {active ? (
        <>
          <video
            ref={videoRef}
            ...
            muted
            playsInline
            className="lunge-viewfinder-video"
          />
          <canvas ref={overlayRef} className="lunge-viewfinder-overlay" />
          <canvas ref={canvasRef} style={{ display: "none" }} />
        </>
      ) : (
        <div className="lunge-viewfinder-placeholder">
          <span>📷</span>
          <p>
            Camera turns on when you hit Start — step back so both legs, front
            foot to back foot, fit in frame
          </p>
        </div>
      )}
    </div>
