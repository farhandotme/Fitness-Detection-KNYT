...
import type { Landmark } from "../hooks/useChairPoseSocket";
...
/**
 * Chair Pose is best judged from a side-on view (so knee bend and torso
 * lean are both clearly visible), full-body, so — like `PushupCamera` —
 * this requests a landscape-leaning feed and lets the CSS box adapt to
 * portrait on narrow screens.
 */
export default function ChairPoseCamera({
  active,
  sendFrame,
  skeleton,
  onError,
  ...
  return (
    <div className="chair-pose-viewfinder">
      {active ? (
        <>
          <video
            ref={videoRef}
            ...
            muted
            playsInline
            className="chair-pose-viewfinder-video"
          />
          <canvas ref={overlayRef} className="chair-pose-viewfinder-overlay" />
          <canvas ref={canvasRef} style={{ display: "none" }} />
        </>
      ) : (
        <div className="chair-pose-viewfinder-placeholder">
          <span>📷</span>
          <p>
