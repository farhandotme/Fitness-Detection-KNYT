...
import type { Landmark } from "../hooks/useSingleLegSquatSocket";
...
/**
 * Same capture/overlay plumbing as `LegRaiseCamera` / `PushupCamera`, but
 * a portrait-leaning frame — the ideal single-leg-squat shot is
 * front-facing (or slight front-oblique) showing the full standing body
 * top to bottom, which is a taller-than-wide frame rather than the
 * landscape framing floor exercises use.
 */
export default function SingleLegSquatCamera({
  active,
  sendFrame,
  skeleton,
  onError,
      ...
      try {
        stream = await navigator.mediaDevices.getUserMedia({
          video: {
            facingMode: "user",
            width: { ideal: 640 },
            height: { ideal: 854 }, // portrait-leaning — full standing body, front-on
  ...
  return (
    <div className="squat-viewfinder">
      {active ? (
        <>
          <video
            ref={videoRef}
            ...
            muted
            playsInline
            className="squat-viewfinder-video"
          />
          <canvas ref={overlayRef} className="squat-viewfinder-overlay" />
          <canvas ref={canvasRef} style={{ display: "none" }} />
        </>
      ) : (
        <div className="squat-viewfinder-placeholder">
          <span>📷</span>
          <p>
            Camera turns on when you hit Start — stand front-on so your
            whole body, standing leg to head, is visible
          </p>
        </div>
      )}
    </div>
