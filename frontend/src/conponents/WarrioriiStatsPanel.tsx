import type { WarriorIIData } from "../hooks/useWarrioriiSocket";

interface Props {
  data: WarriorIIData | undefined;
}
...
export default function WarriorIIStatsPanel({ data }: Props) {
  ...
  return (
    <div className="arm-panel plank-panel">
      <div className="arm-panel-head">
        <span className="arm-panel-label">WARRIOR II</span>
        <span
      ...
      <div className="plank-state-row">
        ...
        {data?.front_leg && (
          <span className="plank-side-pill">{data.front_leg} leg forward</span>
        )}
      </div>
      ...
      <div className="arm-grid">
        ...
        <div className="arm-grid-item">
          <span className="k">Back leg</span>
          <span className="v">
        ...
        <div className="arm-grid-item">
          <span className="k">Stance ratio</span>
          <span className="v">
            {data?.stance_ratio != null ? data.stance_ratio.toFixed(2) : "—"}
          </span>
        </div>
        <div className="arm-grid-item">
          <span className="k">Torso lean</span>
          <span className="v">
            {data?.torso_lean != null ? `${data.torso_lean.toFixed(1)}°` : "—"}
          </span>
        </div>
      </div>
      ...
      <div className="arm-grid" style={{ marginTop: 4 }}>
        ...
        <div className="arm-grid-item">
          <span className="k">Left arm tilt</span>
          <span className="v">
            {data?.left_arm_tilt != null
              ? `${data.left_arm_tilt.toFixed(1)}°`
              : "—"}
          </span>
        </div>
        <div className="arm-grid-item">
          <span className="k">Right arm tilt</span>
          <span className="v">
            {data?.right_arm_tilt != null
              ? `${data.right_arm_tilt.toFixed(1)}°`
              : "—"}
          </span>
        </div>
        <div className="arm-grid-item">
          <span className="k">Wingspan ratio</span>
          <span className="v">
            {data?.wingspan_ratio != null
              ? data.wingspan_ratio.toFixed(2)
              : "—"}
          </span>
        </div>
        <div className="arm-grid-item">
      ...
      >
        {data?.framing_ok === false && data.framing_message
          ...
          : "Position: good — facing camera, wide stance, both arms in frame"}
      </div>
