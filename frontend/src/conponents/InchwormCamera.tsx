...
import type { Landmark } from "../hooks/useInchwormSocket";
...
/**
 * Inchworms swing between standing (tall, portrait-ish silhouette) and a
 * full plank (wide, landscape silhouette) within the same rep, so — like
 * the push-up camera — this requests a roughly 4:3 landscape-leaning feed
 * that keeps the whole body in frame at both ends of the movement, filmed
 * from the side so the walk-out/walk-back is fully visible.
 */
export default function InchwormCamera({
  active,
  sendFrame,
  skeleton,
  onError,
      ...
      try {
        stream = await navigator.mediaDevices.getUserMedia({
          video: {
            facingMode: "user",
            width: { ideal: 854 },
            height: { ideal: 640 }, // landscape-leaning — fits both standing and plank
      ...
      {active ? (
        <>
          <video
            ...
            muted
            playsInline
            className="iw-viewfinder-video"
          />
          <canvas ref={overlayRef} className="iw-viewfinder-overlay" />
          <canvas ref={canvasRef} style={{ display: "none" }} />
        </>
      ) : (
        <div className="iw-viewfinder-placeholder">
          <span>📷</span>
          <p>
            Camera turns on when you hit Start — stand side-on to the
            camera with your full body visible, feet hip-width apart
          </p>
        </div>
      )}
    </div>
