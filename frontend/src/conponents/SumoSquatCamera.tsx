...
import type { Landmark } from "../hooks/useSumoSquatSocket";
...
/**
 * Sumo squats are judged standing, facing the camera, with the whole body
 * (shoulders down to feet) in frame — the same portrait-leaning framing as
 * a regular squat, not the landscape framing push-ups use. Requests a
 * roughly 3:4 portrait feed and lets the CSS box adapt on wide screens.
 */
export default function SumoSquatCamera({
  active,
  sendFrame,
  skeleton,
  onError,
      ...
      try {
        stream = await navigator.mediaDevices.getUserMedia({
          video: {
            facingMode: "user",
            width: { ideal: 640 },
            height: { ideal: 854 }, // portrait-leaning — full standing body in frame
      ...
      {active ? (
        <>
          <video
            ...
            muted
            playsInline
            className="sumo-viewfinder-video"
          />
          <canvas ref={overlayRef} className="sumo-viewfinder-overlay" />
          <canvas ref={canvasRef} style={{ display: "none" }} />
        </>
      ) : (
        <div className="sumo-viewfinder-placeholder">
          <span>📷</span>
          <p>
            Camera turns on when you hit Start — stand facing the camera in
            a wide sumo stance, full body visible
          </p>
        </div>
      )}
    </div>
