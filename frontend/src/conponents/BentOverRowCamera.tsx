...
import type { Landmark } from "../hooks/useBentOverRowSocket";
...
/**
 * Bent-over rows are best read from the side, where the hinge angle and
 * the elbow's up-and-back path are both clearly visible — so this
 * requests the same landscape-leaning feed the other standing/floor
 * exercises use, with the reminder to film from the side baked into the
 * placeholder copy.
 */
export default function BentOverRowCamera({
  active,
  sendFrame,
  skeleton,
  onError,
      ...
      {active ? (
        <>
          <video
            ...
            muted
            playsInline
            className="bor-viewfinder-video"
          />
          <canvas ref={overlayRef} className="bor-viewfinder-overlay" />
          <canvas ref={canvasRef} style={{ display: "none" }} />
        </>
      ) : (
        <div className="bor-viewfinder-placeholder">
          <span>📷</span>
          <p>
            Camera turns on when you hit Start — stand side-on to the camera,
            feet shoulder-width, dumbbells in hand
          </p>
        </div>
      )}
    </div>
