...
import type { Landmark } from "../hooks/useStandingCrossCrunchSocket";
...
/**
 * Standing full-body exercise, so — unlike the landscape-leaning push-up
 * camera — this requests a portrait-leaning feed: the person needs to be
 * visible head to feet for the knee-height math to mean anything.
 */
export default function StandingCrossCrunchCamera({
  active,
  sendFrame,
  skeleton,
  onError,
      ...
      try {
        stream = await navigator.mediaDevices.getUserMedia({
          video: {
            facingMode: "user",
            width: { ideal: 640 },
            height: { ideal: 960 }, // portrait-leaning — full standing body needs to fit
  ...
  return (
    <div className="ccrunch-viewfinder">
      {active ? (
        <>
          <video
            ref={videoRef}
            ...
            muted
            playsInline
            className="ccrunch-viewfinder-video"
          />
          <canvas ref={overlayRef} className="ccrunch-viewfinder-overlay" />
          <canvas ref={canvasRef} style={{ display: "none" }} />
        </>
      ) : (
        <div className="ccrunch-viewfinder-placeholder">
          <span>📷</span>
          <p>
            Camera turns on when you hit Start — stand facing the camera,
            far enough back that your whole body is visible, hands clasped
            behind your head.
          </p>
        </div>
      )}
