...
import type { Landmark } from "../hooks/useSideLegSwingSocket";
...
/**
 * Side leg swings are judged standing, facing (or back-on to) the camera,
 * with the whole body in frame so the sideways leg movement reads as
 * clean horizontal displacement — the same portrait-leaning framing as
 * the sumo squat camera, not push-up's landscape framing.
 */
export default function SideLegSwingCamera({
  active,
  sendFrame,
  skeleton,
  onError,
  ...
  return (
    <div className="legswing-viewfinder">
      {active ? (
        <>
          <video
            ref={videoRef}
            ...
            muted
            playsInline
            className="legswing-viewfinder-video"
          />
          <canvas ref={overlayRef} className="legswing-viewfinder-overlay" />
          <canvas ref={canvasRef} style={{ display: "none" }} />
        </>
      ) : (
        <div className="legswing-viewfinder-placeholder">
          <span>📷</span>
          <p>
            Camera turns on when you hit Start — stand facing the camera,
            full body visible, ready to swing one leg out to the side
          </p>
        </div>
      )}
    </div>
