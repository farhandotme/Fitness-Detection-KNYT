...
import type { Landmark } from "../hooks/useStandingForwardFoldSocket";
 ...
 * portrait-biased resolution — a standing forward fold starts standing tall
 * and hinges down, so the useful frame is tall (head-to-floor down the
 * height), not wide like the plank/side-plank profile shots.
 */
export default function StandingForwardFoldCamera({
  active,
  sendFrame,
  skeleton,
  onError,
      ...
      try {
        stream = await navigator.mediaDevices.getUserMedia({
          video: {
            facingMode: "user",
            width: { ideal: 600 },
            height: { ideal: 800 }, // portrait — need full standing height in frame
  ...
  return (
    <div className="forward-fold-viewfinder">
      {active ? (
        <>
          <video
            ref={videoRef}
            ...
            muted
            playsInline
            className="forward-fold-viewfinder-video"
          />
          <canvas
            ref={overlayRef}
            className="forward-fold-viewfinder-overlay"
          />
          <canvas ref={canvasRef} style={{ display: "none" }} />
        </>
      ) : (
        <div className="forward-fold-viewfinder-placeholder">
          <span>📷</span>
          <p>
            Camera turns on when you hit Start — stand side-on to the camera,
            full body head to floor in frame
          </p>
        </div>
      )}
    </div>
