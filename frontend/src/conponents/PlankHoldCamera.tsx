...
import type { Landmark } from "../hooks/usePlankHoldSocket";
 ...
 * landscape-biased resolution — a plank is judged from a side-on profile
 * view, so the useful frame is wide (head-to-feet across the width), not
 * tall like the standing front-facing exercises.
 */
export default function PlankHoldCamera({
  active,
  sendFrame,
  skeleton,
  onError,
      ...
      try {
        stream = await navigator.mediaDevices.getUserMedia({
          video: {
            facingMode: "user",
            width: { ideal: 854 },
            height: { ideal: 480 }, // landscape — plank is judged side-on
    ...
    <div className="plank-viewfinder">
      {active ? (
        ...
        <div className="plank-viewfinder-placeholder">
          <span>📷</span>
          <p>
            Camera turns on when you hit Start — turn side-on to the camera in
            forearm plank position, head to feet in frame
          </p>
        </div>
      )}
    </div>
