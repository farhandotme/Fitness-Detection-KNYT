...
import type { Landmark } from "../hooks/useDownwardDogSocket";
...
/**
 * Same capture pipeline as PlankHoldCamera — a downward dog is also judged
 * from a side-on profile view (wrist to ankle needs to be in frame for the
 * hip-fold / elevation / arm-line checks), so this requests the same
 * landscape-biased resolution rather than the tall front-facing framing
 * used by the standing exercises.
 */
export default function DownwardDogCamera({
  active,
  sendFrame,
  skeleton,
  onError,
      ...
      try {
        stream = await navigator.mediaDevices.getUserMedia({
          video: {
            facingMode: "user",
            width: { ideal: 854 },
            height: { ideal: 480 }, // landscape — downward dog is judged side-on
  ...
  return (
    <div className="downdog-viewfinder">
      {active ? (
        <>
          <video
            ref={videoRef}
            ...
            muted
            playsInline
            className="downdog-viewfinder-video"
          />
          <canvas ref={overlayRef} className="downdog-viewfinder-overlay" />
          <canvas ref={canvasRef} style={{ display: "none" }} />
        </>
      ) : (
        <div className="downdog-viewfinder-placeholder">
          <span>🐕</span>
          <p>
            Camera turns on when you hit Start — turn side-on to the camera,
            hands and feet on the floor, hips lifted, whole body in frame
          </p>
        </div>
      )}
    </div>
