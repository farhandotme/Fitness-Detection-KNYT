import type { ArmData } from "../hooks/usebicepCurlSocket";
import AngleGauge from "./AngleGauge";

interface Props {
  label: string;
  data: ArmData | undefined;
  compact?: boolean;
}
...
export default function ArmStatsPanel({ label, data, compact }: Props) {
  const angle = data?.smoothed_angle ?? data?.angle ?? null;
  const quality = data?.rep_form_quality;

  return (
    <div className={`arm-panel ${compact ? "compact" : ""}`}>
      <div className="arm-panel-head">
        <span className="arm-panel-label">{label}</span>
      ...
      <div className="arm-panel-rep-row">
        ...
        <span className={`stage-badge ${data?.stage ?? "down"}`}>{(data?.stage ?? "down").toUpperCase()}</span>
      </div>

      <AngleGauge angle={angle} upThreshold={UP_ANGLE} downThreshold={DOWN_ANGLE} stage={data?.stage ?? "down"} compact={compact} />
