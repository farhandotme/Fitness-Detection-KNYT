...
import type { Landmark } from "../hooks/useHollowHoldSocket";
...
/**
 * A hollow hold is judged side-on, lying flat on a mat with the whole
 * body — head to feet, arms overhead — stretched out across the frame.
 * That's a wide, landscape-leaning shot (like push-up's side framing),
 * not the portrait framing a standing exercise needs.
 */
export default function HollowHoldCamera({
  active,
  sendFrame,
  skeleton,
  onError,
      ...
      try {
        stream = await navigator.mediaDevices.getUserMedia({
          video: {
            facingMode: "user",
            width: { ideal: 960 },
            height: { ideal: 540 }, // wide landscape — full body stretched out, side-on
  ...
  return (
    <div className="hollow-viewfinder">
      {active ? (
        <>
          <video
            ref={videoRef}
            ...
            muted
            playsInline
            className="hollow-viewfinder-video"
          />
          <canvas ref={overlayRef} className="hollow-viewfinder-overlay" />
          <canvas ref={canvasRef} style={{ display: "none" }} />
        </>
      ) : (
        <div className="hollow-viewfinder-placeholder">
          <span>📷</span>
          <p>
            Camera turns on when you hit Start — lie on your back, side-on
            to the camera, whole body head to feet in frame
          </p>
        </div>
      )}
    </div>
