...
import type { Landmark } from "../hooks/useWallSitSocket";
 ...
 * portrait-leaning resolution — a wall sit is judged from a front-facing
 * (or slight front-angle) view, with the person standing upright against
 * a wall, so the useful frame is tall (head to feet), not wide like the
 * side-on plank cameras.
 */
export default function WallSitCamera({
  active,
  sendFrame,
  skeleton,
  onError,
      ...
      try {
        stream = await navigator.mediaDevices.getUserMedia({
          video: {
            facingMode: "user",
            width: { ideal: 480 },
            height: { ideal: 854 }, // portrait — wall sit is judged front-on, head to feet
  ...
  return (
    <div className="wallsit-viewfinder">
      {active ? (
        <>
          <video
            ref={videoRef}
            ...
            muted
            playsInline
            className="wallsit-viewfinder-video"
          />
          <canvas ref={overlayRef} className="wallsit-viewfinder-overlay" />
          <canvas ref={canvasRef} style={{ display: "none" }} />
        </>
      ) : (
        <div className="wallsit-viewfinder-placeholder">
          <span>🪑</span>
          <p>
            Camera turns on when you hit Start — stand facing the camera with
            your back against the wall, head to feet in frame
          </p>
        </div>
      )}
    </div>
