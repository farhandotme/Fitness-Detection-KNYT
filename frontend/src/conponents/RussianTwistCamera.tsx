...
import type { Landmark } from "../hooks/useRussianTwistSocket";
...
/**
 * Russian twists are seated and filmed front-on (or slight angle), the
 * camera roughly at chest/torso height facing the user — closer to how
 * the jab is framed (upright, camera-facing) than the side-on push-up.
 * So this requests a portrait-leaning feed sized around the seated torso
 * rather than the push-up's landscape-leaning full-body frame.
 */
export default function RussianTwistCamera({
  active,
  sendFrame,
  skeleton,
  onError,
      ...
      try {
        stream = await navigator.mediaDevices.getUserMedia({
          video: {
            facingMode: "user",
            width: { ideal: 720 },
            height: { ideal: 960 }, // portrait-leaning — seated, torso-height framing
  ...
  return (
    <div className="rtwist-viewfinder">
      {active ? (
        <>
          <video
            ref={videoRef}
            ...
            muted
            playsInline
            className="rtwist-viewfinder-video"
          />
          <canvas ref={overlayRef} className="rtwist-viewfinder-overlay" />
          <canvas ref={canvasRef} style={{ display: "none" }} />
        </>
      ) : (
        <div className="rtwist-viewfinder-placeholder">
          <span>📷</span>
          <p>
            Camera turns on when you hit Start — sit facing the camera with your
            knees bent and torso leaned back slightly
          </p>
        </div>
      )}
    </div>
