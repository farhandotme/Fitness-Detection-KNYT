...
import type { Landmark } from "../hooks/usePeacockPoseSocket";
...
/**
 * Peacock Pose is judged from a side-on view, body extended roughly
 * horizontal — same landscape framing convention as Plank Hold.
 */
export default function PeacockPoseCamera({
  active,
  sendFrame,
  skeleton,
  onError,
  ...
  return (
    <div className="peacock-viewfinder">
      {active ? (
        <>
          <video
            ref={videoRef}
            ...
            muted
            playsInline
            className="peacock-viewfinder-video"
          />
          <canvas ref={overlayRef} className="peacock-viewfinder-overlay" />
          <canvas ref={canvasRef} style={{ display: "none" }} />
        </>
      ) : (
        <div className="peacock-viewfinder-placeholder">
          <span>📷</span>
          <p>
            Camera turns on when you hit Start — side-on view, hands to feet
            fully in frame
          </p>
        </div>
      )}
