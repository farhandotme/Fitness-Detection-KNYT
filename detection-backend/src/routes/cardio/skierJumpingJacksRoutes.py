...
from src.detectors.cardio.skier_jumping_jacks import SkierJumpingJacksSession
...
def _query_int(websocket: WebSocket, name: str, default: int, lo: int, hi: int) -> int:
    ...
    own whether that plan has been completed — `SkierJumpingJacksSession`
...
def _log_rep_progress(label: str, result: dict, exercise_already_logged: bool) -> bool:
    ...
    edge-triggered flag from `SkierJumpingJacksAnalyzer`.
    """
...
@router.websocket("/skier_jumping_jacks")
async def skier_jumping_jacks(websocket: WebSocket):
    await websocket.accept()

    print("Client connected: Skier Jumping Jacks")
    ...
    counter = SkierJumpingJacksSession(
    ...
    try:
        ...
        while True:
            ...
            exercise_logged = _log_rep_progress(
                "Skier Jumping Jacks", result, exercise_logged
            )
        ...
        print("Disconnected: Skier Jumping Jacks")
