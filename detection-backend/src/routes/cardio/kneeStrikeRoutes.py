...
from src.detectors.cardio.knee_strike import KneeStrikeSession
...
@router.websocket("/knee_strike")
async def knee_strike(websocket: WebSocket):
    """Analyze base64 camera frames for standing knee strikes (either leg)."""
    await websocket.accept()
    print("Client connected: KneeStrike")
    ...
    counter = KneeStrikeSession(target_reps, target_sets, set_number)
    ...
    try:
        ...
        while True:
            ...
            if result.get("rep_completed"):
                print(
                    f"[KneeStrike] Rep {result.get('rep_count')}/{result.get('target_reps')} "
