...
from src.detectors.cardio.star_jumps import StarJacksSession
...
def _query_int(websocket: WebSocket, name: str, default: int, lo: int, hi: int) -> int:
    ...
    Same convention as every other route in this app — the coach-assigned
    plan (reps per set / number of sets / which set this connection is for)
    reaches the backend this way, and only `StarJacksSession` decides
...
def _log_rep_progress(label: str, result: dict, exercise_already_logged: bool) -> bool:
    ...
    attempt (no jump / no leg spread / no arm raise), and one line when the
...
@router.websocket("/star-jack")
async def star_jacks(websocket: WebSocket):
    await websocket.accept()

    print("Client connected: Star Jacks")
    ...
    try:
        ...
        while True:
            ...
            exercise_logged = _log_rep_progress("Star Jacks", result, exercise_logged)
