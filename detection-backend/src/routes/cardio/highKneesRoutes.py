...
from src.detectors.cardio.high_knees import HighKneeSession
...
def _query_int(websocket: WebSocket, name: str, default: int, lo: int, hi: int) -> int:
    ...
    own whether that plan has been completed — HighKneeSession is the only
...
def _log_rep_progress(label: str, result: dict, exercise_already_logged: bool) -> bool:
    ...
    HighKneeAnalyzer is already an edge-triggered flag (True only on the
    ...
    if result.get("rep_completed"):
        ...
        leg = result.get("rep_leg") or "n/a"
        rpm = result.get("reps_per_minute")
        print(
            f"[{label}] Rep {rep_count}/{target_reps} "
            f"(set {set_number}/{target_sets}, leg={leg}) — "
            f"quality={quality} tempo={tempo} pace={rpm or 'n/a'}rpm"
        )
...
@router.websocket("/high_knees")
async def high_knees(websocket: WebSocket):
    await websocket.accept()

    print("Client connected: High Knees")
    ...
    try:
        ...
        while True:
            ...
            exercise_logged = _log_rep_progress("High Knees", result, exercise_logged)
