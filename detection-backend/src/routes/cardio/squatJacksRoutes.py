...
from src.detectors.cardio.squat_jacks import SquatJacksSession
...
def _query_int(websocket: WebSocket, name: str, default: int, lo: int, hi: int) -> int:
    ...
    `SquatJacksSession` is the only thing that sets `session_complete` /
...
@router.websocket("/squat_jacks")
async def squat_jacks(websocket: WebSocket):
    await websocket.accept()

    print("Client connected: Squat Jacks")
    ...
    try:
        ...
        while True:
            ...
            exercise_logged = _log_rep_progress("Squat Jacks", result, exercise_logged)
        ...
        print("Disconnected: Squat Jacks")
