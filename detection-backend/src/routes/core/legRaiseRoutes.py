...
def _query_int(websocket: WebSocket, name: str, default: int, lo: int, hi: int) -> int:
    ...
    whether that plan has been completed — `LegRaiseSession` is the only
    thing that sets `session_complete` / `exercise_complete`.
    """
...
def _query_variation(websocket: WebSocket) -> str:
    raw = (websocket.query_params.get("variation") or "").strip().lower()
    return raw if raw in ("straight", "bent_knees") else "straight"
...
@router.websocket("/leg_raise")
async def leg_raise(websocket: WebSocket):
    await websocket.accept()

    print("Client connected: Leg Raise")
    ...
    variation = _query_variation(websocket)

    counter = LegRaiseSession(
    ...
    try:
        ...
        while True:
            ...
            exercise_logged = _log_rep_progress("LegRaise", result, exercise_logged)
