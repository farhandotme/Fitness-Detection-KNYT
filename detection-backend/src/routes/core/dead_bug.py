...
def _framing_feedback(points: list[_Point]) -> Optional[str]:
    ...
    if width > BBOX_TOO_CLOSE or height > BBOX_TOO_CLOSE:
        return "You're too close to the camera — back up so your full reach fits in frame."
...
class _LimbTracker:
    ...
    def update(self, t: float, a, b, c) -> None:
        ...
        self.smoothed_angle = (
            raw if self.smoothed_angle is None else 0.6 * raw + 0.4 * self.smoothed_angle
        )
    ...
    def update(
        ...
        if self.stage == "rest":
            ...
            if not both_extended:
                ...
                duration = (t - self.start_time) if self.start_time is not None else None
                reason = None
                if duration is None or not (MIN_REP_DURATION <= duration <= MAX_REP_DURATION):
                    reason = "tempo"
...
class DeadBugAnalyzer:
    ...
    def __init__(self, target_reps: Optional[int] = None):
        ...
        self.diag_rl = _DiagonalTracker("right_arm_left_leg", self.right_arm, self.left_leg)
        self.diag_lr = _DiagonalTracker("left_arm_right_leg", self.left_arm, self.right_leg)
    ...
    def update(self, landmarks, timestamp_ms: int) -> dict[str, Any]:
        ...
        if landmarks is None or not _looks_like_a_person(landmarks):
            response["feedback"] = "No person detected — lie down sideways to the camera."
            return response
        ...
        is_lying_down = torso_incline is not None and torso_incline <= TORSO_INCLINE_MAX_DEG
        ...
        if not is_lying_down:
            ...
            response["stance_message"] = "Hold your tabletop position steady to start counting…"
        ...
        if self.ready:
            ...
            for limb, pts in (
                ...
                if _visible(pts):
                    raw = _angle_deg(*pts)
                    limb.smoothed_angle = (
                        raw if limb.smoothed_angle is None else 0.6 * raw + 0.4 * limb.smoothed_angle
                    )

        rl_active = self.diag_rl.stage == "extended"
        lr_active = self.diag_lr.stage == "extended"

        rl_result = self.diag_rl.update(t, mid_hip, torso_length, other_diag_active=lr_active)
        lr_result = self.diag_lr.update(t, mid_hip, torso_length, other_diag_active=rl_active)
        ...
        for diag_label, r in (
            ...
            if r["rep_completed"]:
                ...
                side = "right arm + left leg" if diag_label == "right_arm_left_leg" else "left arm + right leg"
                feedback = f"Nice — {side} counted."
            elif r["reason"] is not None:
                response["invalid_attempt"] = True
                response["invalid_reason"] = r["reason"]
                feedback = _INVALID_MESSAGES.get(r["reason"], "That attempt didn't count — reset to tabletop and try again.")
        ...
        if feedback is None:
            feedback = "Good tabletop — reach one arm and the opposite leg out together."
