...
from src.detectors.core.alternating_toe_touch import AlternatingToeTouchSession
...
def _query_int(websocket: WebSocket, name: str, default: int, lo: int, hi: int) -> int:
    ...
    `squatJacksRoutes.py` — the coach-assigned plan (reps per set / number
    of sets / which set this connection is for) reaches the backend this
    way; the frontend does NOT get to decide on its own whether that plan
    has been completed — `AlternatingToeTouchSession` is the only thing
    that sets `session_complete` / `exercise_complete` in the response.
    """
...
@router.websocket("/alternating_toe_touch")
async def alternating_toe_touch(websocket: WebSocket):
    await websocket.accept()

    print("Client connected: Alternating Toe Touch")
    ...
    counter = AlternatingToeTouchSession(
    ...
    try:
        ...
        while True:
            ...
            exercise_logged = _log_rep_progress(
                "Alternating Toe Touch", result, exercise_logged
            )
        ...
        print("Disconnected: Alternating Toe Touch")
