...
def _query_int(websocket: WebSocket, name: str, default: int, lo: int, hi: int) -> int:
    ...
    `FlutterKicksSession` is the only thing that sets `session_complete` /
...
def _log_rep_progress(label: str, result: dict, exercise_already_logged: bool) -> bool:
    ...
    edge-triggered flag (True only on the one frame a swap lands), so we
    ...
    if result.get("rep_completed"):
        ...
        landed_on = result.get("elevated_leg") or "n/a"
        print(
            f"[{label}] Rep {rep_count}/{target_reps} "
            f"(set {set_number}/{target_sets}) — leg={landed_on} "
...
@router.websocket("/flutter_kicks")
async def flutter_kicks(websocket: WebSocket):
    await websocket.accept()

    print("Client connected: Flutter Kicks")
    ...
    try:
        ...
        while True:
            ...
            exercise_logged = _log_rep_progress(
                "Flutter Kicks", result, exercise_logged
            )
        ...
        print("Disconnected: Flutter Kicks")
