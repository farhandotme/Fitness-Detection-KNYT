...
def _query_int(websocket: WebSocket, name: str, default: int, lo: int, hi: int) -> int:
    ...
    Same convention as the other hold-based routes — the coach-assigned
    ...
    whether that plan has been completed — `PeacockPoseSession` is the
...
def _log_hold_progress(label: str, result: dict, exercise_already_logged: bool) -> bool:
    """Print exactly one line whenever the target is reached, and one line
...
@router.websocket("/peacock_pose")
async def peacock_pose(websocket: WebSocket):
    await websocket.accept()

    print("Client connected: Peacock Pose")
    ...
    try:
        ...
        while True:
            ...
            if result.get("hold_state") != last_hold_state:
                print(
                    f"[Peacock Pose] state -> {result.get('hold_state')} "
            ...
            exercise_logged = _log_hold_progress(
                "Peacock Pose", result, exercise_logged
            )
        ...
        print("Disconnected: Peacock Pose")
