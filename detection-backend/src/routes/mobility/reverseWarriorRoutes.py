...
def _query_int(websocket: WebSocket, name: str, default: int, lo: int, hi: int) -> int:
    ...
    has been completed — `ReverseWarriorSession` is the only thing that sets
...
@router.websocket("/reverse_warrior")
async def reverse_warrior(websocket: WebSocket):
    await websocket.accept()

    print("Client connected: Reverse Warrior")
    ...
    try:
        ...
        while True:
            ...
            if result.get("hold_state") != last_hold_state:
                print(
                    f"[Reverse Warrior] state -> {result.get('hold_state')} "
                    f"(held {result.get('hold_seconds')}s / {result.get('target_seconds')}s, "
                    f"set {result.get('set_number')}/{result.get('target_sets')}, "
                    f"front_side={result.get('front_side')})"
                )
                last_hold_state = result.get("hold_state")

            exercise_logged = _log_hold_progress("Reverse Warrior", result, exercise_logged)
        ...
        print("Disconnected: Reverse Warrior")
