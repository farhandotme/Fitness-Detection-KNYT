...
def _query_int(websocket: WebSocket, name: str, default: int, lo: int, hi: int) -> int:
    ...
    Same convention as `sidePlankRoutes.py` / `wallSitRoutes.py` — the
    coach-assigned plan (hold seconds per set / number of sets / which
    set) reaches the backend this way; the frontend does NOT get to
    decide on its own whether that plan has been completed —
    `HalfMoonSession` is the only thing that sets `session_complete` /
...
def _query_support_mode(websocket: WebSocket) -> str:
    ...
    "block". There's no object detection in this codebase to see an
    actual wall/block, so the frontend tells us which one is in use, the
...
@router.websocket("/half_moon")
async def half_moon(websocket: WebSocket):
    await websocket.accept()

    print("Client connected: Half Moon Pose")
    ...
    try:
        ...
        while True:
            ...
            if result.get("hold_state") != last_hold_state:
                print(
                    f"[Half Moon] state -> {result.get('hold_state')} "
            ...
            exercise_logged = _log_hold_progress("Half Moon", result, exercise_logged)
        ...
        print("Disconnected: Half Moon Pose")
