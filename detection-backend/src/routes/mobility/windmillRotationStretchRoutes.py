...
def _query_int(websocket: WebSocket, name: str, default: int, lo: int, hi: int) -> int:
    ...
    Same convention as pushupRoutes.py: the frontend supplies the
    coach-assigned plan (reps per set / sets / which set), and only
    WindmillSession decides whether that plan has been completed.
    """
...
def _log_rep_progress(label: str, result: dict, exercise_already_logged: bool) -> bool:
    ...
    (True only on the one frame a rep lands)."""
    if result.get("rep_completed"):
        ...
        side = result.get("rep_side") or "n/a"
...
@router.websocket("/windmill")
async def windmill(websocket: WebSocket):
    await websocket.accept()

    print("Client connected: Windmill Rotation Stretch")
    ...
    try:
        ...
        while True:
            ...
            exercise_logged = _log_rep_progress("Windmill", result, exercise_logged)
        ...
        print("Disconnected: Windmill Rotation Stretch")
