...
def _query_int(websocket: WebSocket, name: str, default: int, lo: int, hi: int) -> int:
    ...
    has been completed — `StandingForwardFoldSession` is the only thing
...
@router.websocket("/standing_forward_fold")
async def standing_forward_fold(websocket: WebSocket):
    await websocket.accept()

    print("Client connected: Standing Forward Fold")
    ...
    counter = StandingForwardFoldSession(
    ...
    try:
        ...
        while True:
            ...
            if result.get("hold_state") != last_hold_state:
                print(
                    f"[Standing Forward Fold] state -> {result.get('hold_state')} "
            ...
            exercise_logged = _log_hold_progress(
                "Standing Forward Fold", result, exercise_logged
            )
        ...
        print("Disconnected: Standing Forward Fold")
