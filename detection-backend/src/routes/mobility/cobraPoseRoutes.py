...
from src.detectors.mobility.cobra_pose import CobraPoseSession
...
def _query_int(websocket: WebSocket, name: str, default: int, lo: int, hi: int) -> int:
    ...
    has been completed — `CobraPoseSession` is the only thing that sets
...
@router.websocket("/cobra_pose")
async def cobra_pose(websocket: WebSocket):
    await websocket.accept()

    print("Client connected: Cobra Pose")
    ...
    try:
        ...
        while True:
            ...
            if result.get("hold_state") != last_hold_state:
                print(
                    f"[Cobra Pose] state -> {result.get('hold_state')} "
            ...
            exercise_logged = _log_hold_progress("Cobra Pose", result, exercise_logged)
