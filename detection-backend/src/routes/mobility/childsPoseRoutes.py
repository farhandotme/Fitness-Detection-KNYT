...
def _query_int(websocket: WebSocket, name: str, default: int, lo: int, hi: int) -> int:
    ...
    Same convention as `supermanRoutes.py` / `sidePlankRoutes.py` — the
    ...
    `ChildsPoseSession` is the only thing that sets `session_complete` /
...
@router.websocket("/childs_pose")
async def childs_pose(websocket: WebSocket):
    await websocket.accept()

    print("Client connected: Child's Pose")
    ...
    try:
        ...
        while True:
            ...
            if result.get("hold_state") != last_hold_state:
                print(
                    f"[ChildsPose] state -> {result.get('hold_state')} "
            ...
            exercise_logged = _log_hold_progress("ChildsPose", result, exercise_logged)
        ...
        print("Disconnected: Child's Pose")
