...
from src.detectors.mobility.side_leg_swing import SideLegSwingSession
...
def _query_int(websocket: WebSocket, name: str, default: int, lo: int, hi: int) -> int:
    ...
    Same convention as `sumoSquatRoutes.py` — the coach-assigned plan
    reaches the backend this way. The frontend does NOT get to decide on
    its own whether that plan has been completed — `SideLegSwingSession`
...
def _log_rep_progress(label: str, result: dict, exercise_already_logged: bool) -> bool:
    ...
    if result.get("rep_completed"):
        ...
        print(
            ...
            f"(set {set_number}/{target_sets}) — leg={leg} quality={quality} tempo={tempo}"
        )
...
@router.websocket("/side_leg_swing")
async def side_leg_swing(websocket: WebSocket):
    await websocket.accept()

    print("Client connected: SideLegSwing")

    target_reps = _query_int(websocket, "target_reps", default=12, lo=1, hi=100)
    ...
    try:
        ...
        while True:
            ...
            exercise_logged = _log_rep_progress("SideLegSwing", result, exercise_logged)
        ...
        print("Disconnected: SideLegSwing")
