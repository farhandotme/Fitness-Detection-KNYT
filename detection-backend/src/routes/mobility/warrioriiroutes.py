...
def _query_int(websocket: WebSocket, name: str, default: int, lo: int, hi: int) -> int:
    ...
    set) reaches the backend this way; the frontend does NOT get to decide
    on its own whether that plan has been completed — `WarriorIISession`
...
def _log_hold_progress(label: str, result: dict, exercise_already_logged: bool) -> bool:
    """Print exactly one line whenever the hold reaches its target, and one
    line when the exercise finishes — never per-frame.
...
@router.websocket("/warrior_ii")
async def warrior_ii(websocket: WebSocket):
    await websocket.accept()

    print("Client connected: Warrior II")
