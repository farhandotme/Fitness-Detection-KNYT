...
from src.detectors.full_body.squat_thrusts import SquatThrustSession
...
@router.websocket("/squat_thrusts")
async def squat_thrusts(websocket: WebSocket):
    await websocket.accept()

    print("Client connected: Squat Thrusts")
    ...
    try:
        ...
        while True:
            ...
            exercise_logged = _log_rep_progress("SquatThrusts", result, exercise_logged)

            await websocket.send_json(result)
            await asyncio.sleep(0.001)

    except WebSocketDisconnect:
        print("Disconnected: Squat Thrusts")
