...
from src.detectors.full_body.superman import SupermanSession
...
def _query_int(websocket: WebSocket, name: str, default: int, lo: int, hi: int) -> int:
    ...
    Same convention as `plankRoutes.py` / `sidePlankRoutes.py` — the
    ...
    `SupermanSession` is the only thing that sets `session_complete` /
...
def _log_hold_progress(label: str, result: dict, exercise_already_logged: bool) -> bool:
    """Print one line whenever the target is reached, and one line when
...
@router.websocket("/superman")
async def superman(websocket: WebSocket):
    await websocket.accept()

    print("Client connected: Superman")
    ...
    try:
        ...
        while True:
            ...
            if result.get("hold_state") != last_hold_state:
                print(
                    f"[Superman] state -> {result.get('hold_state')} "
            ...
            exercise_logged = _log_hold_progress("Superman", result, exercise_logged)
