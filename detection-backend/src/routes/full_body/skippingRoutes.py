...
from src.detectors.full_body.skipping import SkippingSession
...
def _query_int(websocket: WebSocket, name: str, default: int, lo: int, hi: int) -> int:
    ...
    This is how the coach-assigned plan (jumps per set / number of sets /
    ...
    own whether that plan has been completed — `SkippingSession` is the
...
def _log_rep_progress(label: str, result: dict, exercise_already_logged: bool) -> bool:
    """Print exactly one line per completed jump, and one line when the
    exercise finishes — never per-frame. `rep_completed` is already an
    edge-triggered flag from `SkippingAnalyzer`.
    """
    if result.get("rep_completed"):
        print(
            f"[{label}] Jump {result.get('rep_count')}/{result.get('target_reps')} "
    ...
    if result.get("exercise_complete") and not exercise_already_logged:
        print(
            ...
            f"{result.get('target_sets')} sets x {result.get('target_reps')} jumps done."
        )
        return True
...
@router.websocket("/skipping")
async def skipping(websocket: WebSocket):
    await websocket.accept()

    print("Client connected: Skipping")

    target_reps = _query_int(websocket, "target_reps", default=50, lo=1, hi=1000)
    ...
    try:
        ...
        while True:
            ...
            exercise_logged = _log_rep_progress("Skipping", result, exercise_logged)
