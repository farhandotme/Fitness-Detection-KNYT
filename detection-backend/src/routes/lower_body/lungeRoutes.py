...
from src.detectors.lower_body.lunge import LungeSession
...
def _query_int(websocket: WebSocket, name: str, default: int, lo: int, hi: int) -> int:
    ...
    own whether that plan has been completed — LungeSession is the only
...
def _log_rep_progress(label: str, result: dict, exercise_already_logged: bool) -> bool:
    ...
    LungeAnalyzer is already an edge-triggered flag (True only on the one
    frame a rep lands), so we don't need to track our own counter for reps.
...
@router.websocket("/lunge")
async def lunge(websocket: WebSocket):
    await websocket.accept()

    print("Client connected: Lunge")
    ...
    try:
        ...
        while True:
            ...
            exercise_logged = _log_rep_progress("Lunge", result, exercise_logged)
