...
from src.detectors.lower_body.standing_side_leg_raise import StandingSideLegRaiseSession
...
def _query_int(websocket: WebSocket, name: str, default: int, lo: int, hi: int) -> int:
    ...
    own whether that plan has been completed — `StandingSideLegRaiseSession`
...
def _log_rep_progress(label: str, result: dict, exercise_already_logged: bool) -> bool:
    ...
    edge-triggered flag from `StandingSideLegRaiseAnalyzer`.
    """
...
@router.websocket("/standing_side_leg_raise")
async def standing_side_leg_raise(websocket: WebSocket):
    await websocket.accept()

    print("Client connected: Standing Side Leg Raise")
    ...
    counter = StandingSideLegRaiseSession(
    ...
    try:
        ...
        while True:
            ...
            exercise_logged = _log_rep_progress(
                "Standing Side Leg Raise", result, exercise_logged
            )
        ...
        print("Disconnected: Standing Side Leg Raise")
