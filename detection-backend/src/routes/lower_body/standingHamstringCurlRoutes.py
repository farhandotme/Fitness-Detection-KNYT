...
from src.detectors.lower_body.standing_hamstring_curl import (
    StandingHamstringCurlSession,
)
...
def _query_int(websocket: WebSocket, name: str, default: int, lo: int, hi: int) -> int:
    ...
    own whether that plan has been completed — `StandingHamstringCurlSession`
...
def _log_rep_progress(label: str, result: dict, exercise_already_logged: bool) -> bool:
    ...
    edge-triggered flag from `StandingHamstringCurlAnalyzer`.
    """
...
@router.websocket("/standing_hamstring_curl")
async def standing_hamstring_curl(websocket: WebSocket):
    await websocket.accept()

    print("Client connected: Standing Hamstring Curl")
    ...
    counter = StandingHamstringCurlSession(
    ...
    try:
        ...
        while True:
            ...
            exercise_logged = _log_rep_progress(
                "Standing Hamstring Curl", result, exercise_logged
            )
        ...
        print("Disconnected: Standing Hamstring Curl")
