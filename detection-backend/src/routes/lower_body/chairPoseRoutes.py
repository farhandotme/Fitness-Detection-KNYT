...
def _query_int(websocket: WebSocket, name: str, default: int, lo: int, hi: int) -> int:
    ...
    has been completed — `ChairPoseSession` is the only thing that sets
...
def _log_hold_progress(label: str, result: dict, exercise_already_logged: bool) -> bool:
    """Print exactly one line whenever the hold target is reached, and one
...
@router.websocket("/chair_pose")
async def chair_pose(websocket: WebSocket):
    await websocket.accept()

    print("Client connected: Chair Pose")
    ...
    try:
        ...
        while True:
            ...
            if result.get("hold_state") != last_hold_state:
                print(
                    f"[Chair Pose] state -> {result.get('hold_state')} "
            ...
            exercise_logged = _log_hold_progress("Chair Pose", result, exercise_logged)
