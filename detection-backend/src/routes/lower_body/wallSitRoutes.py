...
from src.detectors.lower_body.wall_sit import WallSitSession
...
def _query_int(websocket: WebSocket, name: str, default: int, lo: int, hi: int) -> int:
    ...
    has been completed — `WallSitSession` is the only thing that sets
...
@router.websocket("/wall_sit")
async def wall_sit(websocket: WebSocket):
    await websocket.accept()

    print("Client connected: Wall Sit")
