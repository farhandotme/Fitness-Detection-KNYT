...
def _query_int(websocket: WebSocket, name: str, default: int, lo: int, hi: int) -> int:
    ...
    own whether that plan has been completed — SquatSession is the only
...
def _log_rep_progress(label: str, result: dict, exercise_already_logged: bool) -> bool:
    ...
    SquatAnalyzer is already an edge-triggered flag (True only on the one
...
async def squat(websocket: WebSocket):
    ...
    try:
        ...
        while True:
            ...
            exercise_logged = _log_rep_progress("Squat", result, exercise_logged)
