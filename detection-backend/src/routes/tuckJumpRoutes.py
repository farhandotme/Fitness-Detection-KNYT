...
def _query_int(websocket: WebSocket, name: str, default: int, lo: int, hi: int) -> int:
    ...
    Same convention as `pushupRoutes.py` — the coach-assigned plan (reps per
    set / number of sets / which set this connection is for) reaches the
    backend this way. The frontend does NOT get to decide on its own whether
    that plan has been completed — `TuckJumpSession` is the only thing that
    sets `session_complete` / `exercise_complete` in the response.
    """
...
def _log_rep_progress(label: str, result: dict, exercise_already_logged: bool) -> bool:
    ...
    attempt (didn't jump / didn't tuck), and one line when the exercise
...
@router.websocket("/tuck_jump")
async def tuck_jump(websocket: WebSocket):
    await websocket.accept()

    print("Client connected: Tuck Jump")
    ...
    try:
        ...
        while True:
            ...
            exercise_logged = _log_rep_progress("Tuck Jump", result, exercise_logged)
