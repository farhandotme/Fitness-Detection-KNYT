...
def _query_int(websocket: WebSocket, name: str, default: int, lo: int, hi: int) -> int:
    ...
    Same convention as `sideLegSwingRoutes.py` — the coach-assigned plan
    reaches the backend this way. The frontend does NOT get to decide on
    its own whether that plan has been completed — `FrontLegSwingSession`
...
@router.websocket("/front_leg_swing")
async def front_leg_swing(websocket: WebSocket):
    await websocket.accept()

    print("Client connected: FrontLegSwing")
    ...
    try:
        ...
        while True:
            ...
            exercise_logged = _log_rep_progress(
                "FrontLegSwing", result, exercise_logged
            )
        ...
        print("Disconnected: FrontLegSwing")
