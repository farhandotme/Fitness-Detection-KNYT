...
from src.detectors.upper_body.pike_pushup import PikePushupSession
...
def _query_int(websocket: WebSocket, name: str, default: int, lo: int, hi: int) -> int:
    ...
    own whether that plan has been completed — PikePushupSession is the
...
def _log_rep_progress(label: str, result: dict, exercise_already_logged: bool) -> bool:
    ...
    PikePushupAnalyzer is already an edge-triggered flag (True only on the
...
@router.websocket("/pike_pushup")
async def pike_pushup(websocket: WebSocket):
    await websocket.accept()

    print("Client connected: PikePushup")
    ...
    try:
        ...
        while True:
            ...
            exercise_logged = _log_rep_progress("PikePushup", result, exercise_logged)
