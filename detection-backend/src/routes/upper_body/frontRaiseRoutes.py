...
from src.detectors.upper_body.front_raise import FrontRaiseSession
...
def _log_rep_progress(result: dict, exercise_already_logged: bool) -> bool:
    if result.get("rep_completed"):
        print(
            f"[FrontRaise] Rep {result.get('rep_count')}/{result.get('target_reps')} "
...
@router.websocket("/front_raise")
async def front_raise(websocket: WebSocket):
    """Analyze base64 camera frames for a two-arm dumbbell front raise."""
    await websocket.accept()
    print("Client connected: FrontRaise")
