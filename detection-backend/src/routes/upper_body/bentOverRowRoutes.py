...
def _query_int(websocket: WebSocket, name: str, default: int, lo: int, hi: int) -> int:
    ...
    The coach-assigned plan (reps per set / number of sets / which set this
    connection is for) reaches the backend this way. The frontend sends
    these when it opens the socket; it does NOT get to decide on its own
    whether that plan has been completed — BentOverRowSession is the only
...
def _log_rep_progress(label: str, result: dict, exercise_already_logged: bool) -> bool:
    ...
    BentOverRowAnalyzer is already an edge-triggered flag (True only on
    the one frame a rep lands).
...
@router.websocket("/bent_over_row")
async def bent_over_row(websocket: WebSocket):
    await websocket.accept()

    print("Client connected: BentOverRow")
    ...
    try:
        ...
        while True:
            ...
            exercise_logged = _log_rep_progress("BentOverRow", result, exercise_logged)
        ...
        print("Disconnected: BentOverRow")
