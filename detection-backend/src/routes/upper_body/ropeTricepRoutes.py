...
from src.detectors.upper_body.rope_tricep import RopeTricepSession
...
def _log_rep_progress(result: dict, exercise_already_logged: bool) -> bool:
    if result.get("rep_completed"):
        print(
            f"[RopeTricep] Rep {result.get('rep_count')}/{result.get('target_reps')} "
...
@router.websocket("/rope_tricep")
async def rope_tricep(websocket: WebSocket):
    """Analyze base64 camera frames for a standing rope tricep pushdown."""
    await websocket.accept()
    print("Client connected: RopeTricep")
