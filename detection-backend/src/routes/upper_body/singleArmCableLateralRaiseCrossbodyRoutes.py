...
from src.detectors.upper_body.single_arm_cable_lateral_raise_crossbody import (
    SingleArmCableLateralRaiseCrossbodySession,
)
...
def _log_rep_progress(result: dict, exercise_already_logged: bool) -> bool:
    if result.get("rep_completed"):
        print(
            "[SingleArmCableLateralRaiseCrossbody] "
            f"{result.get('rep_side', 'n/a').capitalize()} rep "
            f"{result.get('rep_count')}/{result.get('target_reps')} "
    ...
    if result.get("exercise_complete") and not exercise_already_logged:
        print(
            "[SingleArmCableLateralRaiseCrossbody] EXERCISE COMPLETE — "
...
@router.websocket("/single_arm_cable_lateral_raise_crossbody")
async def single_arm_cable_lateral_raise_crossbody(websocket: WebSocket):
    """Analyze base64 frames for a single-arm crossbody cable lateral raise."""
    await websocket.accept()
    print("Client connected: SingleArmCableLateralRaiseCrossbody")
    ...
    counter = SingleArmCableLateralRaiseCrossbodySession(
    ...
    try:
        ...
        print("Disconnected: SingleArmCableLateralRaiseCrossbody")
    finally:
        counter.close()
