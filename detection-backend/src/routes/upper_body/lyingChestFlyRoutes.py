...
from src.detectors.upper_body.lying_chest_fly import ChestFlySession
...
@router.websocket("/lying_chest_fly")
async def chest_fly(websocket: WebSocket):
    await websocket.accept()

    print("Client connected: Lying Dumbbell Chest Fly")
    ...
    try:
        ...
        while True:
            ...
            exercise_logged = _log_rep_progress("Chest Fly", result, exercise_logged)

            await websocket.send_json(result)
            await asyncio.sleep(0.001)

    except WebSocketDisconnect:
        print("Disconnected: Lying Dumbbell Chest Fly")
