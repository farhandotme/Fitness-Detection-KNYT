...
from src.detectors.upper_body.dumbbell_pullover import DumbbellPulloverSession
...
def _log_rep_progress(result: dict, exercise_already_logged: bool) -> bool:
    if result.get("rep_completed"):
        print(
            f"[DumbbellPullover] Rep {result.get('rep_count')}/{result.get('target_reps')} "
    ...
    if result.get("exercise_complete") and not exercise_already_logged:
        print(
            "[DumbbellPullover] EXERCISE COMPLETE — "
...
@router.websocket("/dumbbell_pull_over")
async def dumbbell_pullover(websocket: WebSocket):
    """Analyze base64 camera frames for a supine dumbbell pullover session."""
    await websocket.accept()
    print("Client connected: DumbbellPullover")
    ...
    counter = DumbbellPulloverSession(
    ...
    try:
        ...
        print("Disconnected: DumbbellPullover")
    finally:
        counter.close()
