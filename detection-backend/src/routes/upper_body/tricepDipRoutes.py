...
from src.detectors.upper_body.tricep_dip import TricepDipSession
...
def _log_rep_progress(
    ...
    if result.get("rep_completed"):
        print(
            f"[TricepDip] Rep {result.get('rep_count')}/{result.get('target_reps')} "
...
@router.websocket("/tricep_dip")
async def tricep_dip(websocket: WebSocket):
    """Analyze base64 camera frames for a side-view tricep dip session."""
    await websocket.accept()
    print("Client connected: TricepDip")
