...
from src.detectors.upper_body.reverse_fly_dumbbell import ReverseFlySession
...
def _query_int(websocket: WebSocket, name: str, default: int, lo: int, hi: int) -> int:
    ...
    own whether that plan has been completed — `ReverseFlySession` is the
...
def _log_rep_progress(label: str, result: dict, exercise_already_logged: bool) -> bool:
    ...
    edge-triggered flag from `ReverseFlyAnalyzer`.
    """
...
@router.websocket("/dumbbell_reverse_fly")
async def reverse_fly(websocket: WebSocket):
    await websocket.accept()

    print("Client connected: Reverse Fly")
    ...
    try:
        ...
        while True:
            ...
            exercise_logged = _log_rep_progress("Reverse Fly", result, exercise_logged)
        ...
        print("Disconnected: Reverse Fly")
