"""
Jumping jack rep counting + full-body form analysis.

Design
------
`JumpingJackAnalyzer` is a pure, stateful, whole-body analyzer, following the
exact same shape as `SquatAnalyzer`: one shared `PoseEngine` (owned by
`JumpingJackSession`) feeds it the 33-point pose landmark list every frame,
and it has no idea about the camera or the websocket around it.

Rep counting
------------
A single 0-100 "openness" score drives the rep state machine, combining two
independent signals so a person is only credited with a real jumping jack
when BOTH happen together (not just arms, not just legs):

  * arm_frac  — shoulder abduction angle (hip -> shoulder -> wrist), mapped
                from "arms at sides" (0) to "arms overhead" (1).
  * leg_frac  — ankle-to-ankle distance normalized by shoulder width, mapped
                from "feet together" (0) to "feet spread wide" (1).

openness = 100 * weighted_average(arm_frac, leg_frac), each clamped to
[0, 1] before averaging so a hyper-extended limb can't inflate the other's
absence. If only one signal is visible that frame (e.g. legs out of frame),
openness gracefully falls back to whichever signal is available instead of
going blind — see `_combine_openness`.

Hysteresis (like the squat's DOWN_ANGLE/UP_ANGLE) is:
  OPENNESS_CLOSED_THRESH = enter/confirm "closed" (rest) stage
  OPENNESS_OPEN_THRESH   = enter "open" (extended) stage
A rep completes on the "open" -> "closed" transition, exactly mirroring the
squat's "up" -> "down" completion.

Four-phase overlay
-------------------
On top of the two-state `stage` used for hysteresis, a friendlier 4-value
`phase` is derived every frame for the UI overlay:
  "start"        - resting, closed
  "open"         - actively extending outward (openness rising)
  "close"        - actively returning to center (openness falling)
  "rep_complete" - the single frame a rep just completed on

Mistake detection
------------------
Checked live (posture/sync) and at rep-completion (peak-based):
  * arms_not_fully_raised   — peak arm_frac this rep too low
  * legs_not_spread_enough  — peak leg_frac this rep too low
  * bent_elbows             — elbow angle collapsed below straight-arm floor
  * poor_posture            — torso pitched away from the calibrated
                               standing baseline
  * asymmetrical_movement   — left/right arm angle or leg spread diverged
                               too far at any point in the rep
  * half_rep (partial)      — openness rose then fell back without crossing
                               OPEN_THRESH; mirrors the squat's "squat
                               lower" partial-rep nudge, never counted
  * too fast / too slow     — rep duration outside the valid window, not
                               counted (same policy as squat/pushup)

A flawed-form rep still counts (same "perfect or nothing is discouraging"
philosophy as the squat analyzer) but is tagged `rep_form_quality:
"needs_improvement"` with the specific issues, and rolls into a
`flawed_reps` counter alongside `good_reps`.

Scores (0-100, all clamped)
----------------------------
  * form_score       — 100 minus a fixed penalty per detected mistake on the
                        most recently completed rep.
  * rom_score        — how close the rep's peak arm/leg extension got to
                        "full" (100 = fully overhead + fully spread).
  * stability_score  — how little the hip midpoint drifted sideways during
                        the rep, normalized by shoulder width (some vertical
                        bounce from the jump itself is expected and not
                        penalized — only horizontal sway is).
  * sync_score       — how closely the left and right sides moved together
                        (arm-angle symmetry + leg-spread symmetry) during
                        the rep.
Each is reported both as the latest completed rep's value and as a rolling
average (`avg_form_score`, etc.) over the last 10 reps.

Speed / tempo
-------------
`speed_analysis` reports the just-completed rep's duration, a tempo class
(too_fast/fast/good/slow/too_slow — thresholds tuned faster than the squat's
since a jumping jack is a much quicker movement), and a rolling
reps-per-minute estimate from the last 5 rep durations.

Landmark confidence, smoothing, camera framing
-------------------------------------------------
Same posture as squat.py: every landmark used is visibility-gated
(`_visible`), the openness signal is EMA-smoothed to reject jitter, missing
limbs degrade gracefully instead of crashing, and `_framing_feedback` checks
edge-clipping / distance-from-camera / centering every frame (extended here
to also watch the wrists, since jumping jacks raise the arms well above
where a squat ever needs to look).
"""
