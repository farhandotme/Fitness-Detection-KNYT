...
# Heel / foot-index are part of the standard 33-point BlazePose topology
# (indices 29-32) but `leg_raise.py` never needed them, so nothing had
# actually confirmed `poseEngine.py` exports those specific names. Rather
# than risk an ImportError here — which, since `main.py` imports this
# module at startup, could take down every exercise route, not just this
# one — these are defined locally as the fixed, stable indices instead of
# imported. If your `poseEngine.py` already exports these under the same
# names, it's safe to switch back to importing them; these values match.
LEFT_HEEL = 29
RIGHT_HEEL = 30
LEFT_FOOT_INDEX = 31
RIGHT_FOOT_INDEX = 32
