...
class CompetitionEngine {
  ...
  async onParticipantCountChanged(competitionId: string): Promise<void> {
    ...
    if (count >= room.maxParticipants) {
      ...
      // Scheduled-event rooms don't auto-start on fill - they wait for
      // eventScheduler.ts to call forceStart() at the event's scheduledAt.
      if (room.autoStartOnFull) this.startCountdown(competitionId);
    } else {
      await this.emitRoomState(competitionId);
    }
  }
  ...
  /**
   * Starts a room's countdown regardless of whether it's full - used by
   * eventScheduler.ts when a scheduled event's start time arrives and the
   * room has at least the event's configured minParticipants. Idempotent:
   * a room that isn't WAITING/FULL (already started, completed, abandoned)
   * is left alone.
   */
  async forceStart(competitionId: string): Promise<void> {
    const room = await CompetitionModel.findById(competitionId);
    if (!room) return;
    if (room.status !== "WAITING" && room.status !== "FULL") return;
